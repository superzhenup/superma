...
require_once __DIR__ . '/includes/managed_files.php';
...
// 审计修复 P2-14（2026-07-12）：编辑入口的归属权校验，防止越权修改他人小说
if ($isEdit) {
...
function handleCoverUpload(int $novelId, ?array $existingNovel): ?string {
    ...
    if ($existingNovel && !empty($existingNovel['cover_image'])) {
        deleteManagedRelativeFile($existingNovel['cover_image'], 'storage/covers');
    }
...
document.addEventListener('DOMContentLoaded', function() {
    var existingCover = <?= json_encode($v['cover_image'] ?? '', JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
    if (existingCover) {
