...
const CharacterNetwork = {
    ...
    async loadData() {
        ...
        try {
            const response = await fetch('/api/index.php?route=character_network_data&novel_id=' + this.novelId);
            ...
            if (loading) {
                loading.innerHTML = '<div class="cn-overlay"><i class="bi bi-exclamation-triangle cn-overlay-icon"></i>' +
                    '<div class="cn-overlay-text">' + error.message + '</div></div>';
            }
        }
    },
    ...
    renderDetail(node) {
        ...
        if (rels.length > 0) {
            relHtml = '<ul class="cn-rel-list">' + rels.map(r =>
                `<li class="cn-rel-item">
                    <span class="cn-rel-name" onclick="CharacterNetwork.selectNode(${r.id})">${r.name}</span>
                    <span class="cn-rel-type">${r.type}</span>
                </li>`
            ).join('') + '</ul>';
        }
        ...
        detail.innerHTML = `
            <div class="cn-detail-header">
                <div class="cn-detail-avatar ${roleClass}">${node.label.charAt(0)}</div>
                <div>
                    <div class="cn-detail-name">${node.label}</div>
                    <div class="cn-detail-role">${roleNames[roleClass] || roleClass}</div>
                </div>
            </div>
            ${node.title && node.title !== node.label ? `<div class="cn-detail-title"><i class="bi bi-tag me-1"></i>${node.title}</div>` : ''}
    ...
    renderCharList(nodes) {
        ...
        list.innerHTML = sorted.map(n => {
            ...
            return `<div class="cn-char-item" data-id="${n.id}" onclick="CharacterNetwork.selectNode(${n.id})">
                <span class="cn-char-dot ${dotClass}"></span>
                <span class="cn-char-name">${n.label}</span>
                <span class="cn-char-badge">${roleLabel}</span>
            </div>`;
        }).join('');
    },
