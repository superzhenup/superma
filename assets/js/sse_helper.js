    ...
    async _runOnce(attempt) {
      ...
      while (true) {
        ...
        const stallPromise = new Promise((_, reject) => {
          stallTimerId = setTimeout(() => reject(new Error('stall timeout')), STALL_TIMEOUT_MS);
        });
        ...
        try {
          ...
          // 审计修复 L-2（2026-06-22）：race 结束后必须清理 stall 定时器，
          // 否则每次循环都会遗留一个 90s 的 setTimeout，长时间运行会累积大量定时器。
