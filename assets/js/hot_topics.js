(function(){
  ...
  function renderList(data){
    ...
    var html = data.items.map(function(it){
      ...
      return '<div class="ht-card" onclick="openDetail(' + it.id + ')">'
           +   '<div class="ht-card-head">'
           +     '<span class="src">' + esc(srcLbl) + (it.rank_type ? ' · ' + esc(it.rank_type) : '') + '</span>'
           +     '<span class="rank">No.' + (it.rank_no || 0) + '</span>'
           +   '</div>'
           ...
           +   '</div>'
           +   '<div class="ht-card-hot"><i class="bi bi-fire"></i> ' + fmtNum(it.hotness_score) + '</div>'
  ...
  function renderDetail(n){
    ...
    html += '<div style="color:#9090b8;font-size:.85rem;margin-bottom:.8rem">'
         ...
         + (n.rank_type ? ' · ' + esc(n.rank_type) + ' No.' + n.rank_no : '')
    ...
    if (n.source_url) html += '<div style="margin-bottom:.6rem;font-size:.78rem"><a href="' + esc(n.source_url) + '" target="_blank" style="color:#818cf8">官方页 ↗</a></div>';
  ...
  function renderSettings(s){
    ...
    html += '<div class="mb-3">'
         ...
         + '<input type="number" class="form-control" id="setMinConf" min="0" max="100" value="' + (s.min_confidence||50) + '" style="max-width:120px">'
