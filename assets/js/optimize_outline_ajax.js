...
// 批次大小：与后端同源（get_optimize_progress / optimize_outline_ajax 响应的 batch_size），
// 默认 10。审计 P3-E（2026-06-11）：原前后端各自硬编码 10，改配置会导致批次位置换算错位。
let optimizeBatchSize = 10;
...
async function fetchLastOptimized(novelId) {
    try {
        ...
        if (data && typeof data.optimized_chapter !== 'undefined') {
            if (parseInt(data.batch_size, 10) > 0) {
                optimizeBatchSize = parseInt(data.batch_size, 10);
            }
            return {
                chapter:  parseInt(data.optimized_chapter, 10) || 0,   // 章节号（仅展示用）
                // 批次按“有细纲章节的数组位置”切片，恢复必须用位置而非章节号（细纲有空洞时两者不等）
                position: parseInt(data.optimized_position, 10) || 0,
                total:    parseInt(data.total_chapters, 10) || 0
            };
        }
    } catch (err) {
        console.warn('读取优化进度失败，将从头检查:', err.message);
    }
    return { chapter: 0, position: 0, total: 0 };
}
...
async function processBatch(novelId, batchIndex, lastOptimized, progressLabel, batchLog, streamBox) {
    for (let retry = 0; retry <= BATCH_MAX_RETRIES; retry++) {
        ...
        if (retry > 0) {
            ...
            progressLabel.textContent = `第 ${batchIndex * optimizeBatchSize + 1}～${(batchIndex + 1) * optimizeBatchSize} 章网络错误，${delay / 1000}秒后第 ${retry}/${BATCH_MAX_RETRIES} 次重试...`;
        ...
        try {
            ...
            if (batchLog) {
                ...
                const from = result.batch_from || (batchIndex * optimizeBatchSize + 1);
                const to   = result.batch_to   || ((batchIndex + 1) * optimizeBatchSize);
                const icon = result.retryable ? 'bi-exclamation-triangle' : 'bi-x-circle';
                const color = result.retryable ? 'text-warning' : 'text-danger';
                item.innerHTML = `<span class="${color}"><i class="bi ${icon} me-1"></i>第 ${from}～${to} 章跳过（${escHtml(result.message)}）</span>`;
            ...
            if (isNetworkError(fetchErr)) {
                ...
                for (let soft = 0; soft < 20; soft++) {
                    if (!optimizeOutlineAjaxRunning) return null;
                    progressLabel.textContent = `第 ${batchIndex * optimizeBatchSize + 1}～${(batchIndex + 1) * optimizeBatchSize} 章持续网络错误，30秒后第 ${BATCH_MAX_RETRIES + soft + 1} 次重试（点击停止可中断）...`;
            ...
            if (batchLog) {
                ...
                const item = document.createElement('div');
                item.className = 'p-2 border-bottom border-secondary small';
                item.innerHTML = `<span class="text-warning"><i class="bi bi-exclamation-triangle me-1"></i>第 ${batchIndex * optimizeBatchSize + 1}～${(batchIndex + 1) * optimizeBatchSize} 章跳过（网络错误：${escHtml(fetchErr.message)}）</span>`;
                batchLog.appendChild(item);
                batchLog.scrollTop = batchLog.scrollHeight;
            }
            return null;
...
async function optimizeOutlineLogicAjax() {
    ...
    try {
        // 从数据库获取已优化进度
        const prog = await fetchLastOptimized(novelId);
        let lastOptimized = 0;
        if (prog.total > 0 && prog.position >= prog.total) {
            // 上一轮已全部优化完成：从头重新优化（否则恢复逻辑直接跳到末尾，按钮永远“秒完成、修改0章”）
            batchIndex = 0;
            progressLabel.textContent = '上次优化已全部完成，本次从头重新优化...';
            showToast('上次优化已全部完成，本次将从头重新优化', 'info');
        } else if (prog.position > 0) {
            batchIndex = Math.floor(prog.position / optimizeBatchSize);  // 按后端批次大小换算位置恢复
            lastOptimized = prog.chapter;
            progressLabel.textContent = `检测到已优化至第 ${prog.chapter} 章，继续优化...`;
            showToast(`从第 ${prog.chapter} 章之后继续优化`, 'info');
        }
        ...
        while (hasMore && optimizeOutlineAjaxRunning && batchCount < MAX_BATCHES) {
            batchCount++;
            progressLabel.textContent = `正在优化第 ${batchIndex * optimizeBatchSize + 1}～${(batchIndex + 1) * optimizeBatchSize} 章大纲逻辑...`;
            ...
            // 批次成功
            if (parseInt(result.batch_size, 10) > 0) {
                optimizeBatchSize = parseInt(result.batch_size, 10);
            }
            ...
            // 检查是否完成（has_more=false 即最后一批：旧后端不带 completed 字段，这里双保险）
            if (result.completed || !result.has_more) {
            ...
            if (totalChapters > 0 && batchIndex * optimizeBatchSize >= totalChapters) {
                hasMore = false;
            }
