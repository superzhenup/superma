(function () {
  const CreativeAssist = {
    ...
    async loadContext() {
      ...
      const url = new URL(apiRouteUrl('creative_assist', 'action=context'), window.location.href);
    ...
    async saveDirective(showFeedback) {
      ...
      const json = await this.postJson(apiRouteUrl('creative_assist', 'action=save_directive'), {
    ...
    async runQualityCheck() {
      ...
      try {
        const json = await this.postJson(apiRouteUrl('creative_assist', 'action=quality'), {
          novel_id: this.novelId(),
