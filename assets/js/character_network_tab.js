...
const CharacterNetwork = {
    ...
    async loadData() {
        try {
            ...
            // 审计修复 S-1（2026-06-22）：error.message 拼 innerHTML 会引入 DOM XSS
            // （error.message 来自服务端 data.error 字段，恶意服务器或上游注入可携带
            // <script> / 事件属性）。改用 textContent 构造，与 character_network_standalone.js 一致。
            const loading = document.getElementById('character-network-loading');
            loading.innerHTML = '';
            const wrap = document.createElement('div');
            wrap.className = 'text-center text-muted';
