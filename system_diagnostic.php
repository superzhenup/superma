...
document.addEventListener('DOMContentLoaded', function() {
    ...
    byId('run-diagnostic').addEventListener('click', async function() {
        const novelId = byId('novel-select').value;
        const testType = byId('test-type').value;
        ...
        // 显示加载动画
        byId('loading-overlay').style.display = '';
        byId('result-area').style.display = 'none';
        
        // 调用诊断 API
        try {
            const data = await apiGet('system_diagnostic', {
                novel_id: novelId,
                test_type: testType
            });
            byId('loading-overlay').style.display = 'none';
            if (!data || data.ok === false || data.error) {
                alert('诊断失败：' + (data?.msg || data?.error || '未知错误'));
                return;
            }
            byId('result-area').style.display = '';
            renderResults(data);
        } catch (error) {
            byId('loading-overlay').style.display = 'none';
    ...
    function renderResults(data) {
        ...
        byId('health-score-circle').classList.remove('healthy', 'warning', 'critical');
        byId('health-score-circle').classList.add(status);
        ...
        byId('health-status').textContent = statusText[status] || '未知状态';
        
        // 更新统计
        const summary = data.summary || {};
        byId('stat-passed').textContent = summary.passed || 0;
        byId('stat-warnings').textContent = summary.warnings || 0;
        byId('stat-failed').textContent = summary.failed || 0;
        byId('stat-total').textContent = summary.total_tests || 0;
        
        // 渲染详细结果
        const resultsEl = byId('test-results');
        ...
        for (const [category, tests] of Object.entries(data.tests || {})) {
            ...
            let html = `<div class="test-category">
                <div class="test-category-header" style="background: ${color};">
                    <i class="bi bi-folder me-2"></i>${esc(name)}
                </div>`;
            ...
            for (const [testName, result] of Object.entries(tests)) {
                ...
                html += `<div class="test-item ${result.status}">
                    ...
                    <div class="text-muted small mt-1">${esc(result.message || '')}</div>`;
                
                if (result.details && Object.keys(result.details).length > 0) {
                    html += `<div class="test-details">
                        <pre class="mb-0" style="white-space: pre-wrap; word-break: break-all;">${esc(JSON.stringify(result.details, null, 2))}</pre>
                    </div>`;
                }
