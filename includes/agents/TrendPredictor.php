...
class TrendPredictor
{
    ...
    {
        // P1-1：子分析结果存入局部变量传给 calculateOverallHealth，避免其内部重复查询
        ...
        return [
            ...
            'overall_health' => $this->calculateOverallHealth($quality, $emotion, $coolpoint, $foreshadowing),
        ];
    }
    ...
    {
        try {
            // 修复：原分母为全部历史章节，长篇早期低密度永久拖后腿，
            // below_target 随章节数增长几乎不可逆。改为近 30 章滚动窗口。
            $stats = DB::fetch(
                "SELECT
     ...
     * 计算整体健康度
     */
    private function calculateOverallHealth(array $quality, array $emotion, array $coolpoint, array $foreshadowing): array
    {
        $scores = [];
    ...
    /** 审计修复 M-01（2026-06-12）：$metric 白名单校验，防止 SQL 列名注入 */
    private const ALLOWED_METRICS = ['quality_score', 'emotion_score'];
    ...
    private function getRecentValues(string $metric, int $limit): array
    {
        ...
        try {
            ...
            // 审计修复（2026-07-19 H-10）：DESC 取最近 N 章后必须反转为时间升序——
            // calculateSlope 以数组下标为 x 轴，降序会让"质量上升"被算成负斜率，
            // 且 end($values)/recent_avg 会取到最旧一章而非最新一章。
    ...
    private function detectCoolPointRepetition(): ?string
    {
        try {
            ...
            foreach ($counts as $type => $count) {
                if ($count >= 3) {
                    return "爽点类型「{$type}」近5章中出现{$count}次，建议换类型";
                }
            }
