...
class DialogueVoiceChecker
{
    ...
    private function extractDialogues(string $content): array
    {
        ...
        // 修复：去掉裸"道"（会误匹配"知道/道理/一道光"），去重"喝道"。
        // 说话标签里的"道"通常以"说道/笑道/XX道"形式出现，已由其他分支覆盖。
        $dialogueTags = '(?:说道|笑道|喝道|问道|答道|怒道|叹道|冷道|嚷道|喊道|骂道|低声道|轻声道|沉声道|厉声道|淡淡道|平静道|冷冷道|缓缓道|急忙道|大声道|开口道|接着道|接口道)';
    ...
    private function normalizeSpeaker(string $raw): string
    {
        ...
        // 修复：剥离后缀动作词（如"张三皱了皱眉说道"→ 窗口抓到"张三皱了皱眉"→ 此处剥为"张三"）。
        // 原代码只剥前缀代词，后缀动作词残留导致 isset($voiceMap[$speaker]) 失配，常见句式漏检。
        $name = preg_replace(
            '/(?:笑(?:了(?:一笑|笑)?)?|皱(?:了皱)?眉|顿了顿|看[向着]|看了一眼|看了一[眼脸]|摇(?:了摇)?头|点(?:了点)?头|叹(?:了口)?气|沉吟|犹豫|想了想|眯(?:了眯)?眼|咬牙|抿嘴|挑眉|扬眉|蹙眉|低头|抬头|抬眼|垂眸|轻笑|微笑|苦笑|冷笑|转身|起身|挥手|摆手|拱手|抱拳)$/u',
            '',
            $name
        );
