...
class ImprovementEvaluator
{
    ...
    private function calculateIssuesResolved(array $baseline, array $afterEval): int
    {
        ...
        // P0 修复：$baseline 是上一轮迭代结果（含 evaluation/new_evaluation 子键），
        // 原代码直接读顶层 gate_results 恒为空，issues_resolved 恒为 0。
        // 优先取上一轮的"改后"评估（new_evaluation），即本轮的真实基准。
        $baselineGates = $baseline['new_evaluation']['gate_results']
            ?? $baseline['evaluation']['gate_results']
        ...
        foreach ($iterationResults as $result) {
            ...
            if ($improvement > $best['improvement']) {
                $best = [
                    ...
                    'improvement' => $improvement,
                    'score_change' => ($result['before_score'] ?? null) !== null && ($result['after_score'] ?? null) !== null
    ...
    private function extractImprovementPatterns(array $iterationResults): array
    {
        ...
        // 找出最容易改进的维度
        // 修复（2026-07-22）：usort 会丢失关联键名，导致下游 array_key_first 取到
        // 'total_occurrences' 而非维度名。排序前注入 dimension 字段以保留维度名。
        $sortedPatterns = [];
        foreach ($patterns as $gateName => $pattern) {
            $sortedPatterns[] = ['dimension' => $gateName] + $pattern;
        }
    ...
    private function generateSuggestions(array $overall): array
    {
        ...
        if (!empty($overall['patterns']['easiest_to_improve'])) {
            ...
            // 修复（2026-07-22）：usort 后键名丢失导致 array_key_first 取到 'total_occurrences'，
            // 改为读取 extractImprovementPatterns 注入的 dimension 字段。
            $dimName = is_array($easiest) ? ($easiest['dimension'] ?? '未知') : '未知';
            $suggestions[] = [
                'priority' => 'low',
                'category' => 'focus',
                'suggestion' => "{$dimName}维度最容易改进，可以优先关注",
            ];
        }
