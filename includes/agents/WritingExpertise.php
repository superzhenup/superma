...
class WritingExpertise
{
            ...
            'subtypes' => [
                'opening_weak' => [
                    ...
                    'opening_types' => [
                        ...
                        '冲突式' => '直接进入矛盾冲突',
                        '反转式' => '先给出意外信息再解释',
                        // 修复：原第二个 '悬念式' 重复键覆盖了第一个，
                        // "用未解之谜开场"静默丢失。改为 '疑问式' 区分。
                        '疑问式' => '抛出读者想知道答案的问题',
                    ],
                ],
                'middle_sagging' => [
     ...
     * 题材特定建议
     */
    // 修复：原拼写 GENGE_ADVICE → GENRE_ADVICE（内部引用同步修正）
    private const GENRE_ADVICE = [
        '玄幻' => [
    ...
    public static function getAdvice(string $issueType, array $context = []): array
    {
        ...
        if (!empty($context['genre'])) {
            $genreAdvice = self::GENRE_ADVICE[$context['genre']] ?? null;
        }
