...
class PlotPatternDetector
{
    ...
    private function detectElementOveruse(array $signatures): ?array
    {
        ...
        // 修复：原 max(6, total*0.35) 与注释"或"语义相反——小窗口（如 5 章）下
        // threshold=6 永远不可达，element_overuse 永不触发。改为 min 取较小者，
        // 确保小窗口下 35% 仍可触发，大窗口下封顶 6 次；同时设下限 2 防误报。
        $threshold = max(2, min(6, (int)ceil($total * 0.35))); // 单一类型超过 35% 或 6 次
