...
class OptimizationAgent extends BaseAgent
{
    ...
    private function evaluateOptimizationProposals(array $opportunities): array
    {
        ...
        foreach ($opportunities as $opp) {
            ...
            if ($proposal) {
                ...
                // 修复：补 priority 键——AgentCoordinator::collectAllDirectives 用
                // $opt['priority'] ?? 5 取优先级，缺失时所有 optimization 指令恒为 5，
                // 冲突消解时无法体现 ROI 排序结果。此处由 ROI 映射优先级。
                $proposal['priority'] = match(true) {
                    $proposal['roi'] > 0.2 => 8,
                    $proposal['roi'] > 0.1 => 6,
                    default => 5,
                };
    ...
    {
        try {
            // 修复：原 SQL 对全部历史章节做 SUM(tokens_used) 全表聚合，无时间/章节窗口。
            // 长篇写到几百章后每轮决策都全扫描。改为近 50 章滚动窗口（与性能分析方法对齐）。
            $stats = DB::fetch(
                'SELECT SUM(tokens_used) as total_tokens, COUNT(*) as chapter_count
                 FROM (
                     SELECT tokens_used FROM chapters
                     WHERE novel_id = ? AND status = "completed"
        ...
        try {
            ...
            $stats = DB::fetch(
                'SELECT AVG(quality_change) as avg_change,
                 ...
                 FROM (
                     SELECT o.quality_change, o.evaluated_at
