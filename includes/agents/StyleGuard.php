...
class StyleGuard
{
    ...
    // 阈值集中管理：原为方法内硬编码，现抽为类常量便于统一调整。
    // 如需运行时可调，可进一步包装为 getSystemSetting 读取。
    /** 最小正文字数（低于此值跳过检测） */
    private const MIN_CONTENT_LENGTH = 500;
    /** 对话标签最高频占比阈值（超过则报"标签单一"） */
    private const DIALOGUE_TAG_DOMINANCE = 0.6;
    /** 对话标签最小计数（低于此值不检测单一性） */
    private const DIALOGUE_TAG_MIN_COUNT = 5;
    /** 风格漂移检测间隔（章） */
    private const STYLE_DRIFT_INTERVAL = 20;
    /** 风格漂移检测最小章节 */
    private const STYLE_DRIFT_MIN_CHAPTER = 25;
    /** 风格漂移变化阈值（占比变化超过此值报漂移） */
    private const STYLE_DRIFT_CHANGE_THRESHOLD = 0.25;
    /** 视觉描写占比阈值（超过则报"视觉过度"） */
    private const VISUAL_RATIO_THRESHOLD = 0.7;
    ...
    private const AI_PATTERNS = [
        ...
        'dialogue_tag_monotony' => [
            // 审计修复（2026-07-19 H-中8）：原正则裸「道」字匹配一切"知道/道理/一道光"，
            // 产生的误报计数使每章几乎必报"对话标签单一"。改为完整双字标签列表，去掉裸字符。
            'regex' => '/(说道|笑道|喝道|问道|答道|怒道|叹道|冷道|低声道|轻声道|沉声道|淡淡道)/u',
            // 删除死配置 max_unique_ratio=0.5：代码实际用 DIALOGUE_TAG_DOMINANCE=0.6 判断，
            // 此键从未被读取，保留只会误导维护者以为阈值是 0.5。
            'label' => '对话标签单一',
    ...
    public function detectAIPatterns(string $content): array
    {
        ...
        if ($charCount < self::MIN_CONTENT_LENGTH) return ['issues' => [], 'suggestions' => [], 'total_issues' => 0];
        ...
        foreach (self::AI_PATTERNS as $key => $rule) {
            ...
            if ($key === 'three_layer_emotion') {
                ...
                // 检查是否同一标签占比过高
                if ($count > self::DIALOGUE_TAG_MIN_COUNT) {
                    $tags = [];
                    ...
                    if ($count > 0 && ($mostFrequent / $count) > self::DIALOGUE_TAG_DOMINANCE) {
    ...
    {
        if ($currentChapter % self::STYLE_DRIFT_INTERVAL !== 0 || $currentChapter < self::STYLE_DRIFT_MIN_CHAPTER) return null;
        ...
        foreach (self::STYLE_DIMENSIONS as $dim) {
            ...
            if ($change > self::STYLE_DRIFT_CHANGE_THRESHOLD) {
    ...
    {
        $charCount = mb_strlen($content);
        if ($charCount < self::MIN_CONTENT_LENGTH) {
        ...
        if ($visualRatio > self::VISUAL_RATIO_THRESHOLD) {
            $nonVisual = [];
