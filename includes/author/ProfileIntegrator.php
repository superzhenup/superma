...
class ProfileIntegrator
{
    ...
    private function loadProfileData(): void
    {
        ...
        $this->styleGuide = self::sanitizePromptText((string)($data['style_guide'] ?? ''));
    ...
    public function applyToNovel(int $novelId, int $profileId): bool
    {
        ...
        // 审计修复 P2-19（2026-07-12）：清洗风格文案，降低 prompt injection 风险
        // novels.writing_style 会在 prompt.php 多处直接插入 AI prompt，
        // 若用户在画像里写 "忽略上述指令" 之类的注入文本会被原样注入。
        $styleGuide = self::sanitizePromptText($profile->generateStyleGuide());
    ...
    /**
     * 清洗用户输入的"风格提示"文本，降低 prompt injection 风险
     * 审计修复 P2-19（2026-07-12）
     *
     * 处理策略：
     *   - 移除控制字符（保留 \n \t \r）
     *   - 长度截断（默认 2000 字符）
     *   - 中和常见注入前缀：用【】包裹让其语义化为"数据"而非"指令"
     *     不删除而选择包裹，保留用户原意同时阻止 AI 误读为系统指令
     *
     * @param string $text 用户输入的原始文本
     * @param int $maxLength 最大字符数
     * @return string 清洗后的文本
     */
    public static function sanitizePromptText(string $text, int $maxLength = 2000): string
    {
        if ($text === '') return '';
        // 移除控制字符（保留 \n \r \t）
        ...
        // 中和常见 prompt injection 模式
        $injectionPatterns = [
            '/(?i)\b(ignore\s+(?:all\s+|previous\s+|prior\s+)?instructions?)\b/',
            '/(?i)\b(disregard\s+(?:all\s+|previous\s+)?(?:instructions?|prompts?))\b/',
            '/忽略(?:上述|以上|前面|前面所有|前面所有)?(?:指令|提示)/u',
            '/(?i)^\s*(system|assistant|user)\s*:/m',
            '/(?i)\breset\s+your\s+instructions?\b/',
            '/(?i)\bact\s+as\s+(?:a\s+|an\s+)?(?:different|new)\b/',
            '/(?i)\bdo\s+not\s+follow\b/',
            '/(?i)\byou\s+are\s+(?:now|actually)\b/',
        ];
        foreach ($injectionPatterns as $pattern) {
            $text = preg_replace($pattern, '【$0】', $text) ?? $text;
        }
        return trim($text);
    }
    ...
    public function buildStyleSection(): string
    {
        ...
        // 审计修复 P2-19（2026-07-12）：用户输入的 *_prompt 字段经清洗后再入 prompt
        $habitsPrompt = self::sanitizePromptText($data['writing_habits_prompt'] ?? '');
        $narrativePrompt = self::sanitizePromptText($data['narrative_style_prompt'] ?? '');
        $sentimentPrompt = self::sanitizePromptText($data['sentiment_prompt'] ?? '');
        $creativePrompt = self::sanitizePromptText($data['creative_identity_prompt'] ?? '');
    ...
    public function buildNarrativeGuidance(): string
    {
        ...
        $data = $this->profile->toArray();
        // 审计修复 P2-19（2026-07-12）：用户输入字段经清洗后再入 prompt
        $narrativePrompt = self::sanitizePromptText($data['narrative_style_prompt'] ?? '');
        if ($narrativePrompt) {
            return "【叙事手法指导】\n{$narrativePrompt}";
        }
    ...
    public function buildEmotionalGuidance(): string
    {
        ...
        $data = $this->profile->toArray();
        $sentimentPrompt = self::sanitizePromptText((string)($data['sentiment_prompt'] ?? ''));
        if ($sentimentPrompt) {
            return "【情感基调指导】\n{$sentimentPrompt}";
        }
    ...
    public static function attachToNovel(int $novelId, int $profileId): bool
    {
        try {
            ...
            if (!$integrator->applyToNovel($novelId, $profileId)) {
                return false;
            }
