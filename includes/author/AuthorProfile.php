...
class AuthorProfile
{
    ...
    public function __construct(int $id, bool $loadNow = true)
    {
        $this->id = $id;
        if ($loadNow) {
            $this->load();
    ...
    {
        $profile = new self($id);
        return !empty($profile->data) ? $profile : null;
    }
    ...
    public static function findByUser(?int $userId): array
    {
        ...
        $profiles = $profiles ?: [];
        return array_map(fn($p) => $p->toArray(), self::loadBatch($profiles));
    }
    ...
    public static function listAll(array $filters = [], int $page = 1, int $pageSize = 20): array
    {
        ...
        $sql = 'SELECT * FROM author_profiles WHERE ' . implode(' AND ', $where) . ' ORDER BY updated_at DESC LIMIT ? OFFSET ?';
        $params[] = $pageSize;
        $params[] = $offset;
        ...
        return [
            'items' => array_map(fn($p) => $p->toArray(), self::loadBatch($rows)),
            'total' => $total,
            'page' => $page,
            'page_size' => $pageSize,
    ...
    /**
     * 批量加载画像实例：N+1 → 4 条 IN 查询 + 内存组装。
     * @param array $rows author_profiles 全字段行数组
     * @return self[]
     */
    private static function loadBatch(array $rows): array
    {
        ...
        $habitsRows    = DB::fetchAll("SELECT * FROM author_writing_habits WHERE profile_id IN ($ph)", $ids) ?: [];
        $narrativeRows = DB::fetchAll("SELECT * FROM author_narrative_styles WHERE profile_id IN ($ph)", $ids) ?: [];
        $sentimentRows = DB::fetchAll("SELECT * FROM author_sentiment_analysis WHERE profile_id IN ($ph)", $ids) ?: [];
        $creativeRows  = DB::fetchAll("SELECT * FROM author_creative_identity WHERE profile_id IN ($ph)", $ids) ?: [];

        $indexBy = fn(array $rs) => array_combine(array_column($rs, 'profile_id'), $rs) ?: [];
        $habitsMap    = $indexBy($habitsRows);
        $narrativeMap = $indexBy($narrativeRows);
        $sentimentMap = $indexBy($sentimentRows);
        $creativeMap  = $indexBy($creativeRows);

        return array_map(function ($row) use ($habitsMap, $narrativeMap, $sentimentMap, $creativeMap) {
            $p = new self((int)$row['id'], false);
            $p->data     = $row;
            $p->habits   = $habitsMap[$row['id']] ?? [];
            $p->narrative = $narrativeMap[$row['id']] ?? [];
            $p->sentiment = $sentimentMap[$row['id']] ?? [];
            $p->creative  = $creativeMap[$row['id']] ?? [];
            return $p;
        }, $rows);
    }
    ...
    public function update(array $data): bool
    {
        ...
        try {
            // 审计修复 M-4（2026-07-01）：使用 DB::beginTransaction() 幂等封装，
            // 避免外层已有事务时 $pdo->beginTransaction() 抛 PDOException
            DB::beginTransaction();

            if (isset($updateData['is_default']) && $updateData['is_default']) {
                DB::update('author_profiles', ['is_default' => 0], 'id!=?', [$this->id]);
            }
    ...
    public function delete(): bool
    {
        try {
            DB::beginTransaction();
            // author_uploaded_works 无外键级联，需手动清理
            DB::delete('author_uploaded_works', 'profile_id=?', [$this->id]);
