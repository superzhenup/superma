...
class StatsTracker
{
    ...
    public static function record(int $wordsAdded, int $chaptersAdded = 1): void
    {
        ...
        try {
            // 审计修复（2026-07-19 H-19）：原"先 SELECT 再 UPDATE/INSERT"在并发写作下
            // 存在丢失更新与唯一键冲突丢计数；改为单条原子 upsert。
            DB::execute(
                "INSERT INTO usage_stats (stat_date, words_added, chapters_added, novels_active, reported_at)
                 VALUES (?, ?, ?, 1, NULL)
                 ON DUPLICATE KEY UPDATE
                    words_added = words_added + VALUES(words_added),
                    chapters_added = chapters_added + VALUES(chapters_added),
                    reported_at = NULL",
                [$today, $wordsAdded, $chaptersAdded]
            );
        } catch (\Throwable $e) {
     ...
     *
     * 上报数据清单（隐私声明）：
     *   - site_id: 站点标识符（由管理员配置或自动生成，不含敏感信息）
     *   - date: 统计日期（YYYY-MM-DD）
     *   - words_added / chapters_added / novels_active: 聚合统计数字
     *   - version: 系统版本号
     * 不上报：用户名、密码、API密钥、小说内容、IP地址等任何敏感信息。
     * 此功能默认关闭（STATS_REPORT_ENABLED=false），需管理员主动启用。
     */
    public static function report(): array
    {
        ...
        try {
            ...
            curl_setopt_array($ch, [
                ...
                CURLOPT_SSL_VERIFYPEER => true,   // L-2 修复：显式启用 SSL 验证
                CURLOPT_SSL_VERIFYHOST => 2,
                CURLOPT_PROTOCOLS => CURLPROTO_HTTPS,  // 仅允许 HTTPS
            ]);
    ...
    private static function sendAsync(string $url, array $payload): void
    {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_SSL_VERIFYPEER => true,   // 与 report() 一致：启用 SSL 验证
            CURLOPT_SSL_VERIFYHOST => 2,
            CURLOPT_PROTOCOLS => CURLPROTO_HTTPS,  // 仅允许 HTTPS
        ]);
