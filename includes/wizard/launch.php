...
(function(){
  ...
  btnReview.addEventListener('click', function(){
    ...
    var full = '', first = true;
    fetch(apiRouteUrl('wizard', 'action=review_check'), {
      method:'POST',
  ...
  function saveExtra(showAlert){
    return fetch(apiRouteUrl('wizard', 'action=save_extra'), {
      method:'POST',
      headers:{'Content-Type':'application/json','X-CSRF-Token': window.CSRF_TOKEN||''},
  ...
  function autoSaveExtra(useBeacon){
    ...
    if (useBeacon) {
      ...
      try {
        fetch(apiRouteUrl('wizard', 'action=save_extra'), {
  ...
  document.getElementById('btn-final-confirm').addEventListener('click', function(){
    ...
    saveExtra(false).then(function(){
      return fetch(apiRouteUrl('wizard', 'action=save_stage'), {
        method:'POST',
        headers:{'Content-Type':'application/json','X-CSRF-Token': window.CSRF_TOKEN||''},
        body: JSON.stringify({novel_id: novelId, stage: 'launch'})
      }).then(function(r){ return r.json(); });
    }).then(function(d){
      if (d && d.ok !== false) location.href = 'create.php?edit=' + novelId;
  ...
  function requestPatch(){
    ...
    fetch(apiRouteUrl('wizard', 'action=review_patch'), {
      method:'POST',
      headers:{'Content-Type':'application/json','X-CSRF-Token': window.CSRF_TOKEN||''},
      body: JSON.stringify({novel_id: novelId})
    })
  ...
  if (adoptApply) adoptApply.addEventListener('click', function(){
    ...
    fetch(apiRouteUrl('wizard', 'action=apply_review'), {
      method:'POST',
      headers:{'Content-Type':'application/json','X-CSRF-Token': window.CSRF_TOKEN||''},
