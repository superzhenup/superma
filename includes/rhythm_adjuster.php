...
class RhythmAdjuster
{
    ...
    public function calculateRhythm(int $currentChapter, array $coolPointHistory = []): array
    {
        ...
        // 审计优化（2026-07-20）：段落长度比例必须在 switch 之后按阶段赋值。
        // 原先写在 case 'setup' 内，导致 climax/ending 分支的阶段判断永假，
        // 非铺垫章永远落在默认 2:5:3。
        if ($stage === 'climax' || $stage === 'ending') {
            $result['paragraph_ratio'] = ['long' => 0.2, 'medium' => 0.4, 'short' => 0.4];
        } elseif ($stage === 'setup') {
            $result['paragraph_ratio'] = ['long' => 0.4, 'medium' => 0.4, 'short' => 0.2];
        } elseif ($stage === 'resolve') {
            $result['paragraph_ratio'] = ['long' => 0.25, 'medium' => 0.45, 'short' => 0.3];
        } else {
            // rising 及未知阶段：中等偏舒展
        ...
        // 计算爽点密度（近5章）
        // 审计修复（2026-07-19 H-中11）：分母固定 /5 导致前 5 章密度天然 ≤0.2，
        // 必然低于 target*0.6 开篇必报"爽点饥饿"。
        // 改为按实际可用窗口章数计算，min(5, currentChapter-1) 避免第1章除零。
        $windowSize = max(1, min(5, $currentChapter - 1));
        $coolPointDensity = $coolPointCount / $windowSize;
        // 审计修复 P2-12（2026-07-12）：优先读取小说级配置，回退到全局默认
        $targetDensity = (float)ConfigCenter::getNovel($this->novelId, 'ws_cool_point_density_target', 0.88, 'float');
    ...
    public function generateRhythmInstructions(array $rhythm): string
    {
        ...
        $lines[] = "段落宜" . match($rhythm['stage'] ?? '') {
            'climax', 'ending' => '短促有力（短段偏多）',
            'setup' => '舒展铺陈（长段可略多）',
            'resolve' => '紧凑推进',
            default => '长短交替',
        } . "；参考长:中:短 = " . (isset($rhythm['paragraph_ratio'])
            ? (round($rhythm['paragraph_ratio']['long'] * 10) . ':' . round($rhythm['paragraph_ratio']['medium'] * 10) . ':' . round($rhythm['paragraph_ratio']['short'] * 10))
            : '3:5:2') . "（软建议）";
        $lines[] = "· 长段落：世界观、心理；中段落：对话、动作；短段落：高潮、爆发";
        $lines[] = "";
