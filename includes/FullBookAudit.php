...
class FullBookAudit
{
    ...
    private static function ensureTable(): void
    {
        ...
        // 审计修复（2026-07-19 H-17）：score 需容纳 0-10，DECIMAL(3,1) 最大 9.9 会在
        // 严格模式下插入失败并导致整份审计报告丢失；对存量表做幂等列宽升级。
        ...
        // 存量表列宽幂等升级：仅当 score 仍为 DECIMAL(3,1) 时执行
        $col = DB::fetch("SHOW COLUMNS FROM `novel_audits` LIKE 'score'");
        if ($col && stripos((string)($col['Type'] ?? ''), 'decimal(3,1)') !== false) {
            DB::execute("ALTER TABLE `novel_audits` MODIFY COLUMN `score` DECIMAL(4,1) DEFAULT NULL");
        }
    ...
    public static function run(int $novelId, int $chNum, ?int $modelId = null): ?array
    {
        try {
            ...
            // 审计修复（2026-07-19 H-17）：钳制到 0-10，防御 AI 返回越界值
            $score   = isset($data['score']) ? max(0, min(10, round((float)$data['score'], 1))) : null;
    ...
    private static function buildCondensedView(int $novelId, int $chNum): string
    {
        ...
        } catch (\Throwable $e) { error_log("FullBookAudit 静默异常: " . $e->getMessage() . " in " . $e->getFile() . ":" . $e->getLine()); }
        ...
        try {
            ...
            if ($arcs) {
                // 审计修复（2026-07-19 H-中9）：原实现 mb_substr($t, 0, 9000) 从头截断，
                // 长书的最新弧段被丢弃，审计视图随书变长而退化。
                // 改为尾部截断：保留最新弧段，早期弧段被压缩。
                $t = '';
                foreach ($arcs as $a) {
