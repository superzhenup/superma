    ...
    private function generateRecommendations(array $alerts): array
    {
        ...
        foreach ($alerts as $alert) {
            $recs[] = match ($alert['type']) {
                ...
                // 修复：ws_agent_enabled 为废弃键，全库仅此一处引用；实际总开关键是
                // agent.enabled（ConfigCenter::get 读 system_settings 表）（2026-07-22）
                'directive_stagnation' => '建议：检查Agent总开关（agent.enabled）和后台任务是否正常运行',
