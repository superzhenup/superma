...
class GlobalNovelController
{
    ...
    /** @var int 触发相位差（章）——与全书圣经（%20===0）错峰，避免同章叠加两个重型 AI 任务 */
    private const TRIGGER_OFFSET = 10;
    ...
    {
        return $currentChapter > 0 && $currentChapter % self::TRIGGER_INTERVAL === self::TRIGGER_OFFSET;
    }
...
PROMPT;
    ...
    private static function checkCoolPointRhythm(int $novelId, int $currentChapter): array
    {
        try {
            ...
            // 检查连续无爽点
            // 审计修复（2026-07-19 H-中1）：原算法中 end($coolChapters) 取到最老爽点章，
            // $lastGap 覆盖所有真实 interval 导致 maxGap 恒等于"当前章−最老章"、几乎必然误报；
            // 且 $prevChapter=$currentChapter+1 使尾部间隔多算 1（off-by-one）。
            if (!$issue && $coolCount > 0) {
                $coolChapters = array_column($recentCoolPoints, 'chapter_number');
                rsort($coolChapters); // 降序：[最新 … 最老]
                // N 个爽点章把这些章节 + 当前章 + 下一章共 N+1 段划分成 N+1 段。
                // prevChapter 从"当前章"开始，循环覆盖爽点之间的间隔。
                $maxGap = 0;
                ...
                // 最老爽点章到第1章的间隔
                $firstGap = (int)$coolChapters[count($coolChapters) - 1] - 1;
