...
class HotNovelService
{
    ...
    public function upsertItem(array $item, string $batchId): array
    {
        ...
        $data = [
            ...
            // 协议白名单清洗：仅存 http(s)，阻断 javascript:/data: 等危险协议（防前端渲染时的存储型 XSS）
            'cover_url'           => HotNovelValidator::sanitizeUrl($item['cover_url'] ?? null),
            'source_url'          => HotNovelValidator::sanitizeUrl($item['source_url'] ?? null),
        ...
        if ($existing) {
            ...
            if (!empty($update)) {
                ...
                $params = [];
                foreach ($update as $k => $v) {
                    // 列名白名单校验，防止 SQL 注入（与 DB::validateColumns 保持一致）
