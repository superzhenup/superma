...
class HotNovelValidator
{
    ...
    /**
     * 清洗外部推送的 URL（source_url / cover_url）。
     * 仅放行 http(s) 绝对地址；javascript:/data:/vbscript: 等危险协议或相对地址一律丢弃返回 null，
     * 防止存储型 XSS——这些字段会被前端渲染进 <a href> / <img src>，而前端 esc() 只做 HTML 实体编码、
     * 不校验协议（如 "javascript:..." 不含被转义字符，会原样进入 href）。
     */
    public static function sanitizeUrl($url): ?string
    {
        $u = trim((string)($url ?? ''));
        if ($u === '') return null;
        return preg_match('#^https?://#i', $u) === 1 ? $u : null;
    }
