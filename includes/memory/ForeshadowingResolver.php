...
class ForeshadowingResolver
{
    ...
    private function scoreAll(array $pending, string $outline): array
    {
        ...
        foreach ($pending as $item) {
            ...
            if ($overlapCount > 0) {
                // M-14 修复（2026-07-25）：原 $overlapCount * 10 使共享 5 个泛词（如"主角""战斗"）
                // 的伏笔得 +50 分，淹没更相关但关键词少的伏笔。降低单词权重并设上限：
                // 单词贡献 4 分，最多 3 个词（+12 分上限），避免泛词堆叠主导排序。
                $keywordBonus = min($overlapCount, 3) * 4;
                $score += $keywordBonus;
