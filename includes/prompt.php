...
// 审计修复（2026-06-22）：本层自带其文档化依赖（helpers.php/data.php），使
// "require prompt.php" 即可独立加载——包括被 ChapterPromptBuilder 顶层 require 的场景。
// 生产路径中三者已先由 functions.php（行17-19 helpers→data→prompt）加载，
// require_once 幂等下为 no-op；helpers.php/data.php/cache.php 顶层均无 DB 或副作用，
// 且不回指 prompt.php，独立加载安全、无循环依赖。
...
/**
 * 审计修复 C-2（2026-06-17）：集中消毒小说用户可控字段，防止 Prompt 注入。
 *
 * 所有进入 AI 提示词的小说字段必须经过此函数。覆盖以下字段：
 *  - title（书名）
 *  - genre / writing_style（类型/风格）
 *  - protagonist_info / protagonist_name（人设）
 *  - plot_settings / world_settings / extra_settings（情节/世界观/自定义）
 *  - ref_author（参考作者）
 *  - tone / audience / style_vector 元素（文风类）
 *
 * 策略：复用 includes/helpers.php::sanitizeForPrompt() 移除已知的越狱/注入
 * 关键词并做长度截断。在每个 prompt builder 入口处对整本小说一次性调用，
 * 避免下游散点遗漏。
 *
 * 注意：仅在已知输入来自 novels 表时调用；不要在用户/AI 消息正文上调用
 * （这些是 AI 的合法输出或用户正文，不应被过滤掉）。
 */
function sanitizeNovelForPrompt(array $novel): array {
    $keys = [
        'title', 'genre', 'writing_style',
        'protagonist_info', 'protagonist_name',
        'plot_settings', 'world_settings', 'extra_settings',
        'ref_author',
    ];
    foreach ($keys as $k) {
        if (isset($novel[$k]) && is_string($novel[$k])) {
            $novel[$k] = sanitizeForPrompt($novel[$k]);
        }
    }
    return $novel;
}
...
function generateWordCountWarnings(int $targetWords): string
{
    $levels = [
        ...
        ['percent' => 80, 'label' => '警告', 'message' => '开始准备收尾，少开新支线'],
        ['percent' => 90, 'label' => '严重', 'message' => '进入收束/钩子段，勿再展开新情节'],
        ['percent' => 95, 'label' => '紧急', 'message' => '尽快收束当前事件，勿新开冲突'],
    ];
...
function buildGlobalBeatStats(int $novelId, int $count): string
{
    ...
    try {
        ...
        return "\n【全书高频套路统计——排名靠前者已被反复使用，本批宜主动换新】\n"
            . implode("\n", $lines)
            . "\n（建议：高频 TOP3 套路在本批{$count}章中各最多再用 1 次；非强制，情节需要时可破例）\n";
    } catch (\Throwable $e) {
        return '';
    }
}
...
function buildOutlinePrompt(
    ...
    // 审计修复 C-2（2026-06-17）：集中消毒用户可控字段，阻断 Prompt 注入。
    $novel = sanitizeNovelForPrompt($novel);
    ...
    $extraSettings   = $truncate($novel['extra_settings']   ?? '', 250);
    ...
    // 弧段摘要（全局历史记忆）——只保留最近 5 段，每段摘要截断
    $arcSection   = '';
    if (!empty($arcSummaries)) {
        $arcLines = [];
        foreach ($arcSummaries as $arc) {
            if ((int)$arc['chapter_to'] < $startChapter) {
                $sum = $truncate((string)($arc['summary'] ?? ''), 80);
                $arcLines[] = "第{$arc['chapter_from']}-{$arc['chapter_to']}章：{$sum}";
            }
        }
        if (!empty($arcLines)) {
            $arcLines = array_slice($arcLines, -5);
            $arcSection = "\n【故事线回顾】\n" . implode("\n", $arcLines) . "\n";
        }
    }

    // 近章局部大纲（微观衔接）——默认 8 章，硬顶 12
    $contextSection = '';
    if (!empty($recentOutlines)) {
        $lines = [];
        $recentShow = max(5, min(12, (int)getSystemSetting('ws_outline_recent_show', 8, 'int')));
        foreach (array_slice($recentOutlines, -$recentShow) as $oc) {
            $summary = safe_substr(trim($oc['outline'] ?? ''), 0, 90);
    ...
    if (!empty($keyEvents)) {
        $lines = [];
        foreach (array_slice($keyEvents, -12) as $e) {
            $ev = $truncate((string)($e['event'] ?? ''), 60);
            $lines[] = "第{$e['chapter']}章：{$ev}";
        }
        $keyEventsSection = "\n【已发生关键事件（禁重复）】\n" . implode("\n", $lines) . "\n";
    }
    ...
    if (!empty($characterStates)) {
        ...
        foreach ($characterStates as $name => $state) {
            ...
            if (!empty($state['status'])) $parts[] = "境：" . $truncate((string)$state['status'], 40);
    ...
    if (!empty($pendingForeshadow)) {
        $sorted = $pendingForeshadow;
        usort($sorted, function ($a, $b) use ($chNum) {
            $da = (int)($a['deadline'] ?? 99999);
            $db = (int)($b['deadline'] ?? 99999);
            $ua = $chNum >= $da - 5 ? 0 : 1;
            $ub = $chNum >= $db - 5 ? 0 : 1;
            return $ua <=> $ub ?: $da <=> $db;
        });
        $lines = [];
        foreach (array_slice($sorted, 0, 8) as $f) {
            $deadline = '';
            if (!empty($f['deadline'])) {
            ...
            $desc = $truncate((string)($f['desc'] ?? ''), 50);
            $lines[] = "第{$f['chapter']}章埋：{$desc}{$deadline}";
        }
        $foreshadowSection = "\n【待回收伏笔】\n" . implode("\n", $lines) . "\n";
    }
    ...
    $momentumSection = $storyMomentum
        ? "\n【故事势能】\n" . $truncate($storyMomentum, 200) . "\n"
        : '';

    $arcSummarySection = $currentArcSummary
        ? "\n【当前弧段摘要】\n" . $truncate($currentArcSummary, 220) . "\n"
        : '';
    ...
    if (!empty($recentHookTypes)) {
        $lines = [];
        $hookShow = max(3, min(8, (int)getSystemSetting('ws_outline_hook_history', 6, 'int')));
    ...
    if (!empty($coolPointHistory)) {
        $coolShow = max(5, min(10, (int)getSystemSetting('ws_outline_cool_history', 8, 'int')));
    ...
    if (!empty($recentCoolTypes)) {
        ...
        if (!empty($bannedNames)) {
            $bannedCoolTypesRule = "禁止使用近3章已用爽点类型：" . implode('、', $bannedNames);
        }
    }
    ...
    if (!empty($existingTitles)) {
        ...
        if ($total <= 60) {
            ...
            // 超过60条时智能截断：最近40条 + 最早15条
            $earliest = array_slice($existingTitles, 0, 15, true);
            $latest   = array_slice($existingTitles, -40, 40, true);
    ...
    try {
        ...
        if (!$storyOutline) {
            // 审计修复 A01（2026-06-16）：用 information_schema 探测列存在性，
            // 替代原 try/catch 双 SELECT 兜底，避免 schema 演进时 prompt 行为漂移。
            // character_progression 是 v48 新增列，老库可能缺失。
            $hasProgression = SchemaMigrator::columnExists(
                DB::connect(),
                'story_outlines',
                'character_progression'
            );
            $selectCols = 'story_arc, act_division, character_arcs, '
                . ($hasProgression ? 'character_progression, ' : '')
                . 'world_evolution, major_turning_points, recurring_motifs';
            try {
                $storyOutline = DB::fetch(
                    "SELECT {$selectCols} FROM story_outlines WHERE novel_id=?",
                    [$novel['id']]
                );
            } catch (\Throwable) {
                $storyOutline = null;
    ...
    $protagonistAnchorRule = $protagonistName
        ? "8. 主角名锚定：大纲中主角必须写作「{$protagonistName}」，不可改名或换称呼"
        : '';
    $noRepeatRule = '禁复述【禁止已发生关键事件】；标题不得与【已用章节标题】相同或高度相似'
        . ($bannedCoolTypesRule !== '' ? "；{$bannedCoolTypesRule}" : '');

    // 审计优化（2026-07-20）：铁律从 22 条压到 8 条硬约束 + 软建议，减少注意力稀释。
    // 条件性要求（逾期伏笔/卷目标/收尾/主线）改由用户区段落承载，「有段则执行」。
    $system = <<<EOT
你是一位专业网文大纲作家。输出纯JSON数组，无前缀后缀无markdown代码块。

【设定锚定】情节须依据下列设定，禁止与设定矛盾：
{$settingAnchor}

【硬约束——必须遵守】
1. 格式：输出恰好{$count}个完整对象；summary≤150字，key_points每条≤30字，hook≤40字
2. 不重复：{$noRepeatRule}
3. 承接与一致：人物职务/身份/处境与【人物状态】一致；有【上章钩子】则本批第1章必须直接承接；相邻章因果相连（前章hook→后章触发）
4. 推进证明：每章必须写清 story_delta、conflict、result、next_trigger、unique_role；hook 须为具体悬念，禁空洞句
5. 钩子类型：{$hookTypeRule}
6. 用户区条件段（有则必须执行）：【严重逾期伏笔】【本卷必须回收】【本卷写作目标】【全书故事主线】【收尾阶段】——按其原文要求安排，不得推诿
7. 禁词与禁结构：禁用"——"及AI高频词（深邃、凝视、缓缓、蓦然、骤然、指节泛白、沉默了几秒、愣在原地等）；禁止把提示词结构写进大纲（如「章末钩子」「铺垫段」字样）
{$protagonistAnchorRule}

【软建议——可权衡】
· 节奏：本批≥1快≥1慢；≥2章 suspense:"有"；爽点排期 {$coolPointSchedule}（相邻不重复）
· 表达：【表达词语防重复】中的词尽量换用替代说法
· 开篇：若含第1章，宜尽早抛核心悬念并给强钩子

EOT;

    // 审计优化（2026-07-24）：用户区可选段总长超预算时，优先丢掉软参考、保留承接/伏笔/人物
    $softBudget = max(6000, (int)getSystemSetting('ws_outline_user_soft_budget', 10000, 'int'));
    $softParts = [
        &$expressionGuide, &$sceneTemplateSection, &$beatStatsSection, &$momentumSection,
        &$arcSummarySection, &$hookHistorySection, &$coolPointSection, &$arcSection,
    ];
    $hardPartsLen = safe_strlen($prevHookSection . $overdueSection . $volumeForceResolve . $volumeSection
        . $progressSection . $endingContext . $characterSection . $foreshadowSection
        . $keyEventsSection . $usedTitlesSection . $contextSection . $storyOutlineSection);
    $softLen = 0;
    foreach ($softParts as $sp) { $softLen += safe_strlen((string)$sp); }
    while (($hardPartsLen + $softLen) > $softBudget && !empty($softParts)) {
        $idx = array_key_last($softParts);
        $softLen -= safe_strlen((string)$softParts[$idx]);
        $softParts[$idx] = '';
        array_pop($softParts);
    }
...
function buildStoryOutlineFromChaptersPrompt(array $novel, array $chaptersData): array {
    // 审计修复 C-2（2026-06-17）：集中消毒所有用户可控字段
    $novel = sanitizeNovelForPrompt($novel);
...
function buildStoryOutlinePrompt(array $novel): array {
    // 审计修复 C-2（2026-06-17）：集中消毒用户可控字段
    $novel = sanitizeNovelForPrompt($novel);
...
function calculateCoolPointSchedule(int $startChapter, int $count, array $history = [], ?int $novelId = null): string
{
    ...
    // 审计修复 P2-12（2026-07-12）：优先读取小说级配置，回退到全局默认
    $densityTarget = (float)($novelId
        ? ConfigCenter::getNovel($novelId, 'ws_cool_point_density_target', 0.88, 'float')
        : getSystemSetting('ws_cool_point_density_target', 0.88, 'float'));
