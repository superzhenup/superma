...
function buildOutlinePrompt(
    ...
    // ── 全书进度感知（新增）────────────────────────────────────────────
    $progressSection = '';
    $endingContext   = '';  // 全书收尾指令（进度 >= 90% 时注入）
    try {
        $engine = new MemoryEngine((int)$novel['id']);
        $prog   = $engine->getProgressContext($startChapter);

        if ($prog['target_chapters'] > 0) {
            $lines = [];

            // 基础进度
            $lines[] = "📖 写作进度：第{$prog['completed_chapters']}章 / 共{$prog['target_chapters']}章"
                . "（{$prog['progress_pct']}%完成，剩余{$prog['remaining_chapters']}章）";

            // 三幕定位
            if ($prog['act_phase']) {
                $lines[] = "🎭 叙事阶段：{$prog['act_phase']}";
            }

            // 卷进度
            if ($prog['volume_progress']) {
                $lines[] = "📚 当前卷：{$prog['volume_progress']}";
            }
            ...
            if ($pendingCnt > 0) {
                $overdueNote = $overdueCnt > 0 ? "（其中{$overdueCnt}条已逾期！）" : '';
                $lines[] = "🧵 待回收伏笔：共{$pendingCnt}条{$overdueNote}";
                foreach ($prog['pending_foreshadowing_list'] as $f) {
                    $lines[] = "  · {$f}";
                }
            }

            // 全书转折点进度
            $turningPoints = $prog['major_turning_points'];
            if (!empty($turningPoints)) {
                $passed  = array_filter($turningPoints, fn($t) => $t['passed']);
                $pending = array_filter($turningPoints, fn($t) => !$t['passed']);
                $lines[] = "🔖 全书转折点：已过 " . count($passed) . " / 共 " . count($turningPoints) . " 个";
                // 最近未过的前2个
                foreach (array_slice(array_values($pending), 0, 2) as $tp) {
                    $lines[] = "  → 待触发（第{$tp['chapter']}章）：{$tp['event']}";
                }
            }

            // 主角成长轨迹
            if (!empty($prog['character_arcs'])) {
                foreach ($prog['character_arcs'] as $charName => $arc) {
                    ...
                    if ($stage) {
                        $lines[] = "👤 {$charName}当前弧线阶段（{$stageLabel}）：{$stage}";
                    }
                }
            }
            ...
            $endingContext = buildEndingContext($prog, $startChapter);
        }
    } catch (\Throwable $e) {
        // 进度感知失败静默跳过，不影响主流程
    }
...
13. hook_type六选一：crisis_interrupt/info_bomb/plot_twist/new_goal/emotional_impact/upgrade_omen，相邻不重复
...
世界观：{$worldSettings}
其他：{$extraSettings}
{$progressSection}{$endingContext}{$volumeSection}{$overdueSection}{$volumeForceResolve}{$arcSection}{$contextSection}{$prevHookSection}{$momentumSection}{$keyEventsSection}{$characterSection}{$foreshadowSection}
输出JSON数组（{$count}个元素）：
...
function buildChapterPrompt(
    ...
    // 容差：优先读用户配置（ws_chapter_word_tolerance），兜底用动态容差
    $configTolerance = (int)getSystemSetting('ws_chapter_word_tolerance', 0, 'int');
    $tolerance = $configTolerance > 0 ? $configTolerance : max(CFG_TOLERANCE_MIN, (int)($targetWords * CFG_TOLERANCE_RATIO));
    $minWords  = $targetWords - $tolerance;
    $maxWords  = $targetWords + $tolerance;
    // 提前收尾触发字数：达到目标字数的指定比例时开始准备收尾
    $earlyFinishWords = (int)($targetWords * CFG_EARLY_FINISH_RATIO);
...
【写作铁律，必须遵守优先级最高】
1. 字数硬性限制（最高优先级）：正文必须严格控制在 {$minWords} ~ {$maxWords} 字之间，绝对不可超过 {$maxWords} 字。写作时请在心中估算字数进度：当已写约 {$earlyFinishWords} 字时，立即启动收尾流程，用钩子段结束本章，禁止在收尾阶段继续延伸新情节。
2. 人物一致性：所有人物的职务、身份、生死状态必须与【人物当前状态】完全一致，不得擅自改变
3. 情节不重复：【全书已发生事件】中出现的任何事件，严禁以任何形式重演或变体重复
4. 逻辑自洽：本章发生的事件必须是前情的自然延伸，因果链条清晰，不得出现无因之果
5. 直接开始：从"第{$chNum}章 {$chapter['title']}"这一行直接开始输出正文，不要有任何前言、后记、解释或"好的，我来写"等废话
...
EOT;
    ...
    $user = <<<EOT
{$arcChapterSection}{$prevSection}{$recentChapterSection}{$tailSection}{$characterSection}{$momentumSection}{$eventsSection}{$semanticSection}{$tropesSection}{$foreshadowSection}{$volumeGoalSection}{$chapterProgressSection}{$chapterEndingContext}{$synopsisSection}【小说信息】
