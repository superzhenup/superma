...
class EndingEnforcer
{
    ...
    public function checkEndingCompliance(string $content): array
    {
        ...
        // 审计修复（2026-07-19 H-中7）：checkEndingCompliance 的 passed 原恒为 true，
        // 全函数无任何置 false 路径。收紧期/结局期应阻止引入新伏笔和新角色。
        // 正常期不检查
        if ($stage === 'normal') {
            return $result;
        }
        ...
        foreach ($foreshadowKeywords as $keyword) {
            if (mb_strpos($content, $keyword) !== false) {
                if ($stage === 'tighten' || $stage === 'ending') {
                    $result['issues'][] = "收尾期检测到新伏笔关键词：「{$keyword}」——应回收而非埋设新伏笔";
                    $result['passed'] = false;
                } else {
        ...
        if ($stage === 'ending') {
            ...
            if ($remaining <= 1 && !$hasEnding) {
                $result['issues'][] = "最终章缺少明确的结局信号（结局/终章/完结等关键词）";
                $result['passed'] = false;
            }
        }
