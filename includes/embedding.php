...
class KnowledgeBase {
     ...
     * @param bool $isAutomaticExtraction  true 时（自动章节提取），更新已有记录时仅在 DB 字段为空时才填充，
     *                                     保护用户手动精修的数据。
     */
    public function saveCharacter(array $data, bool $skipEmbedding = false, bool $isAutomaticExtraction = false): int {
        $id = (int)($data['id'] ?? 0);
        ...
        if ($id > 0) {
            ...
            // v1.11.10: 自动提取场景下，仅当 DB 字段为空时才允许填充，避免覆盖用户精修数据
            if ($isAutomaticExtraction && !empty($filtered)) {
                $filtered = $this->keepOnlyEmptyColumns('novel_characters', $id, $filtered);
            }
            ...
            if (!$skipEmbedding) $this->updateCharacterEmbedding($id);
            return $id;
        } else {
    ...
    /**
     * v1.11.10: 自动提取防覆盖辅助 — 仅保留 DB 中对应字段为空的列，避免覆盖用户精修数据
     * @return array 过滤后的字段集合
     */
    private function keepOnlyEmptyColumns(string $table, int $id, array $data): array {
        if (empty($data)) return [];
        // L-14 修复：表名白名单校验，防止未来调用方传入不受控的表名
        static $allowed = ['novel_characters', 'novel_worldbuilding', 'novel_plots', 'novel_style'];
        if (!in_array($table, $allowed, true)) {
            throw new \InvalidArgumentException("Invalid table for empty-check: {$table}");
        }
        $cols = array_keys($data);
        $colList = implode(',', array_map(fn($c) => "`{$c}`", $cols));
        $row = DB::fetch("SELECT {$colList} FROM `{$table}` WHERE id=? AND novel_id=? LIMIT 1", [$id, $this->novelId]);
        if (!$row) return $data; // 安全降级：查不到记录直接返回原数据
        $out = [];
        foreach ($data as $k => $v) {
            $cur = $row[$k] ?? null;
            // 仅当 DB 字段为空时才填充
            if ($cur === null || $cur === '' || $cur === '0' || $cur === 0) {
                $out[$k] = $v;
            }
        }
        return $out;
    ...
    /**
     * 更新角色的向量
     * v1.11.10: 重构 — 不再接收局部数组，而是直接从 DB 拉取最新完整记录进行向量化，
     * 避免传入的局部 $data（如经 mergeOnlyNonEmpty 过滤后）退化为只剩"名字"的空心向量。
     */
    private function updateCharacterEmbedding(int $characterId): void {
        $row = DB::fetch('SELECT * FROM novel_characters WHERE id=? AND novel_id=? LIMIT 1', [$characterId, $this->novelId]);
        if (!$row) return;
        $text = implode("\n", array_filter([
            "角色名：" . ($row['name'] ?? ''),
            !empty($row['alias'])       ? "别名：" . $row['alias']       : '',
            !empty($row['appearance'])  ? "外貌：" . $row['appearance']  : '',
            !empty($row['personality']) ? "性格：" . $row['personality'] : '',
            !empty($row['background'])  ? "背景：" . $row['background']  : '',
            !empty($row['abilities'])   ? "能力：" . $row['abilities']   : '',
        ]));
    ...
    /**
     * 按 id 直接取单条角色（避免拉全表后 PHP 过滤）
     */
    public function getCharacterById(int $id): ?array {
        $row = DB::fetch(
            'SELECT * FROM novel_characters WHERE id=? AND novel_id=? LIMIT 1',
            [$id, $this->novelId]
        );
    ...
    public function saveWorldbuilding(array $data, bool $skipEmbedding = false, bool $isAutomaticExtraction = false): int {
        $id = (int)($data['id'] ?? 0);
        $name = trim((string)($data['name'] ?? ''));
        ...
        if ($id > 0) {
            ...
            // v1.11.10: 自动提取场景下仅填空，保护用户精修
            if ($isAutomaticExtraction && !empty($filtered)) {
                $filtered = $this->keepOnlyEmptyColumns('novel_worldbuilding', $id, $filtered);
            }
            if (!empty($filtered)) {
                DB::update('novel_worldbuilding', $filtered, 'id=? AND novel_id=?', [$id, $this->novelId]);
            }
            if (!$skipEmbedding) $this->updateWorldbuildingEmbedding($id);
            return $id;
        } else {
    ...
    private function updateWorldbuildingEmbedding(int $id): void {
        // v1.11.10: 从 DB 拉取最新完整记录进行向量化，避免局部更新覆盖
        $row = DB::fetch('SELECT * FROM novel_worldbuilding WHERE id=? AND novel_id=? LIMIT 1', [$id, $this->novelId]);
        if (!$row) return;
        $categoryNames = [
            'location' => '地点',
            'faction' => '势力',
        ...
        $text = implode("\n", array_filter([
            ($categoryNames[$row['category'] ?? 'other'] ?? '其他') . "：" . ($row['name'] ?? ''),
            $row['description'] ?? '',
        ]));
     ...
     * 按 id 直接取单条世界观设定
     */
    public function getWorldbuildingById(int $id): ?array {
        $row = DB::fetch(
            'SELECT * FROM novel_worldbuilding WHERE id=? AND novel_id=? LIMIT 1',
            [$id, $this->novelId]
        );
        return $row !== false ? $row : null;
    }
    ...
    public function savePlot(array $data, bool $skipEmbedding = false, bool $isAutomaticExtraction = false): int {
        $id = (int)($data['id'] ?? 0);
        ...
        if ($id > 0) {
            ...
            // v1.11.10: 自动提取场景下仅填空，保护用户精修
            if ($isAutomaticExtraction && !empty($filtered)) {
                $filtered = $this->keepOnlyEmptyColumns('novel_plots', $id, $filtered);
            }
            if (!empty($filtered)) {
                DB::update('novel_plots', $filtered, 'id=? AND novel_id=?', [$id, $this->novelId]);
            }
            if (!$skipEmbedding) $this->updatePlotEmbedding($id);
            return $id;
        } else {
    ...
    private function updatePlotEmbedding(int $id): void {
        // v1.11.10: 从 DB 拉取最新完整记录进行向量化
        $row = DB::fetch('SELECT * FROM novel_plots WHERE id=? AND novel_id=? LIMIT 1', [$id, $this->novelId]);
        if (!$row) return;
        $text = implode("\n", array_filter([
            "第" . ($row['chapter_from'] ?? 1) . "章：" . ($row['title'] ?? ''),
            $row['description'] ?? '',
        ]));
     ...
     * 按 id 直接取单条情节
     */
    public function getPlotById(int $id): ?array {
        $row = DB::fetch(
            'SELECT * FROM novel_plots WHERE id=? AND novel_id=? LIMIT 1',
            [$id, $this->novelId]
        );
        return $row !== false ? $row : null;
    }
    ...
    public function saveStyle(array $data, bool $skipEmbedding = false, bool $isAutomaticExtraction = false): int {
        $id = (int)($data['id'] ?? 0);
        $name = trim((string)($data['name'] ?? ''));
        ...
        if ($id > 0) {
            ...
            // v1.11.10: 自动提取场景下仅填空，保护用户精修
            if ($isAutomaticExtraction && !empty($filtered)) {
                $filtered = $this->keepOnlyEmptyColumns('novel_style', $id, $filtered);
            }
            if (!empty($filtered)) {
                DB::update('novel_style', $filtered, 'id=? AND novel_id=?', [$id, $this->novelId]);
            }
            if (!$skipEmbedding) $this->updateStyleEmbedding($id);
            return $id;
        } else {
    ...
    private function updateStyleEmbedding(int $id): void {
        // v1.11.10: 从 DB 拉取最新完整记录进行向量化
        $row = DB::fetch('SELECT * FROM novel_style WHERE id=? AND novel_id=? LIMIT 1', [$id, $this->novelId]);
        if (!$row) return;
        $vecParts = [];
        if (!empty($row['vec_style']))    $vecParts[] = '文风：' . $row['vec_style'];
        if (!empty($row['vec_pacing']))   $vecParts[] = '节奏：' . $row['vec_pacing'];
        if (!empty($row['vec_emotion']))  $vecParts[] = '情感：' . $row['vec_emotion'];
        if (!empty($row['vec_intellect']))$vecParts[] = '智慧：' . $row['vec_intellect'];
        if (!empty($row['ref_author']))   $vecParts[] = '参考：' . $row['ref_author'];
        if (!empty($row['keywords']))     $vecParts[] = '高频词：' . $row['keywords'];
     ...
     * 按 id 直接取单条风格
     */
    public function getStyleById(int $id): ?array {
        $row = DB::fetch(
            'SELECT * FROM novel_style WHERE id=? AND novel_id=? LIMIT 1',
            [$id, $this->novelId]
        );
        return $row !== false ? $row : null;
    }
     ...
     */
    public function extractFromChapter(int $chapterNumber, string $content, ?int $chapterId = null): array {
        $stats = [
            'characters' => 0,
            'worldbuilding' => 0,
            'plots' => 0,
            ...
            foreach ($extracted['characters'] as $char) {
                try {
                    $this->saveCharacter([
                        ...
                        'appear_count' => 1,
                    ], true, true); // v1.11.10: skipEmbedding=true + isAutomaticExtraction=true
                    $stats['characters']++;
                } catch (\Throwable $e) {
        ...
        // 存储章节摘要向量（备用）
        // 审计修复（2026-07-19 H-08）：source_type='chapter' 的 source_id 统一为 chapters.id——
        // 清理方（ChapterMutationService 重写/失效级联）全部按 chapters.id 删除，
        // 此前写入 chapter_number 会导致旧向量删不掉、且可能误删他章向量。
        if ($chapterId === null || $chapterId <= 0) {
            $chapterId = (int)(DB::fetchColumn(
        ...
        if ($chapterId > 0) {
            ...
            $this->storeEmbedding('chapter', $chapterId, $summary);
        }

        // v1.11.10: 章节数据完全入库后，统一批量补齐缺失的向量，消除单条实时 HTTP 超时累加
        try {
            $stats['embeddings_backfilled'] = $this->backfillEmbeddings();
        } catch (\Throwable $e) {
            error_log("extractFromChapter backfillEmbeddings failed: " . $e->getMessage());
        }
    ...
    /**
     * v1.11.10: 批量补齐 — 找出当前小说下尚未生成向量（novel_embeddings 缺失）的所有设定，一次性补全。
     * 兑现注释中"backfill"的开发承诺。
     *
     * @return int 实际补齐的条目总数
     */
    public function backfillEmbeddings(): int {
        if (EmbeddingProvider::getConfig() === null) {
            return 0;
        }
        ...
        $totalFilled = 0;
        $sources = [
            'character'     => ['table' => 'novel_characters',   'updater' => 'updateCharacterEmbedding'],
            'worldbuilding' => ['table' => 'novel_worldbuilding','updater' => 'updateWorldbuildingEmbedding'],
            'plot'          => ['table' => 'novel_plots',        'updater' => 'updatePlotEmbedding'],
            'style'         => ['table' => 'novel_style',        'updater' => 'updateStyleEmbedding'],
        ];

        foreach ($sources as $sourceType => $info) {
            // 找出该类型缺失向量的所有条目 id
            $rows = DB::fetchAll(
                "SELECT t.id FROM `{$info['table']}` t
                 LEFT JOIN novel_embeddings e
                   ON e.source_type = ? AND e.source_id = t.id AND e.novel_id = t.novel_id
                 WHERE t.novel_id = ? AND e.id IS NULL",
                [$sourceType, $this->novelId]
            );
            if (empty($rows)) continue;

            foreach ($rows as $r) {
                try {
                    $this->{$info['updater']}((int)$r['id']);
                    $totalFilled++;
                } catch (\Throwable $e) {
                    error_log("backfillEmbeddings {$sourceType} #{$r['id']} failed: " . $e->getMessage());
                }
            }
        }
