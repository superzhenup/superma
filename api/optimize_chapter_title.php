...
try {
    ...
    $__titleResponded = true;
    // 审计修复 P2-15（2026-07-12）：原始错误仅对管理员可见，普通用户收通用消息
    // INTENTIONAL_DIAGNOSTIC_DISCLOSURE：本系统为单用户管理端；管理员可见
    // 模型错误用于配置诊断，完整异常仍只写服务端日志且响应不含文件/行号/堆栈。
    $safeMsg = isAdmin() ? $e->getMessage() : '标题优化失败，请稍后重试';
    error_log('optimize_chapter_title failed: ' . $e->getMessage());
    echo json_encode([
        'ok' => false,
        'error' => $safeMsg,
        'msg' => $safeMsg,
...
function clean_title_suggestion(string $raw): string
{
    ...
    // 审计修复（2026-07-19 H-16）：trim() 的 charlist 按字节处理，多字节引号/书名号
    // 会被拆成字节集合，削掉尾字节落在集合内的汉字（如"局"E5 B1 80 中的 \x80）。
    // 改用 /u 正则按字符裁剪首尾。
    $text = preg_replace(
        '/^[\x00-\x20"\'\x{201C}\x{201D}\x{2018}\x{2019}\x{300A}\x{300B}]+|[\x00-\x20"\'\x{201C}\x{201D}\x{2018}\x{2019}\x{300A}\x{300B}]+$/u',
        '',
        $text
    ) ?? $text;
