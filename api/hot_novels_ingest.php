...
foreach ($items as $idx => $item) {
    ...
    try {
        ...
        // 审计修复 P3-2（2026-07-01）：不向调用方回显异常原文（可能含 DB 列名/约束名），仅记日志
        error_log(sprintf('hot_novels_ingest item %d (title=%s) failed: %s', $idx, $t, $e->getMessage()));
        $failed[] = ['index' => $idx, 'title' => $t, 'reason' => 'VALIDATION_ERROR', 'detail' => '处理失败，详见服务端日志'];
    }
}
