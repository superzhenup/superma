...
require_once $baseDir . '/includes/managed_files.php';
...
// 统一路由入口 api/index.php 不会携带 author_profile.php 后的 PATH_INFO。
// 优先接受 action/query 参数并重建旧 handler 所需的 pathParts，同时继续兼容
// 直接访问 author_profile.php/profile/{id} 的历史客户端。
$queryAction = preg_replace('/[^a-zA-Z0-9_-]/', '', (string)($_GET['action'] ?? ''));
if ($queryAction !== '') {
    $action = $queryAction;
    $profileId = (int)($_GET['profile_id'] ?? $_GET['id'] ?? 0);
    $workId = (int)($_GET['work_id'] ?? $_GET['id'] ?? 0);
    $dimension = preg_replace('/[^a-z_]/', '', (string)($_GET['dimension'] ?? ''));

    if ($action === 'dimension') {
        $pathParts = [$action, $dimension, $profileId];
    } elseif (in_array($action, ['style-guide', 'vector', 'works'], true)) {
        $pathParts = [$action, 'profile', $profileId];
    } elseif ($action === 'work') {
        $pathParts = [$action, $workId];
    } elseif (in_array($action, ['profile', 'detailed-report'], true)) {
        $pathParts = [$action, $profileId];
    } else {
        $pathParts = [$action];
    }
}
...
function requireOwnedAuthorProfile(int $profileId, ?int $userId): AuthorProfile
{
    $profile = AuthorProfile::find($profileId);
    if (!$profile) {
        authorJsonResponse(['error' => 'profile_not_found'], 404);
    }

    $profileData = $profile->toArray();
    $profileUserId = $profileData['user_id'] ?? null;
    if ($userId !== null && $profileUserId !== null && (int)$profileUserId !== (int)$userId) {
        authorJsonResponse(['error' => 'profile_forbidden'], 403);
    }
...
function saveAuthorDimensionResult(int $profileId, string $dimension, array $resultData): void
{
    $dimensionTables = [
    ...
    if (!isset($dimensionTables[$dimension])) {
        throw new InvalidArgumentException('Invalid author dimension: ' . $dimension);
    }

    $rowData = match ($dimension) {
        'writing_habits' => [
            'vocabulary_preference' => json_encode($resultData['vocabulary_preference'] ?? [], JSON_UNESCAPED_UNICODE),
            'word_complexity' => $resultData['word_complexity'] ?? 'moderate',
            'sentence_length_avg' => $resultData['sentence_length_avg'] ?? 0,
            'paragraph_length_avg' => $resultData['paragraph_length_avg'] ?? 0,
            'sentence_patterns' => json_encode($resultData['sentence_patterns'] ?? [], JSON_UNESCAPED_UNICODE),
            'use_passive' => $resultData['use_passive'] ?? 0,
            'use_dialogue' => $resultData['use_dialogue'] ?? 0,
            'rhetorical_devices' => json_encode($resultData['rhetorical_devices'] ?? [], JSON_UNESCAPED_UNICODE),
            'metaphor_frequency' => $resultData['metaphor_frequency'] ?? 'medium',
            'uniqueness_score' => $resultData['uniqueness_score'] ?? 0,
            'confidence' => $resultData['confidence'] ?? 0,
            'source_chapter_count' => $resultData['source_chapter_count'] ?? 0,
        ],
        'narrative_style' => [
            'narrative_pov' => $resultData['narrative_pov'] ?? 'third_limited',
            'pov_switch_frequency' => $resultData['pov_switch_frequency'] ?? 'rare',
            'pacing_type' => $resultData['pacing_type'] ?? 'medium',
            'scene_transition_style' => $resultData['scene_transition_style'] ?? null,
            'tension_curve' => json_encode($resultData['tension_curve'] ?? [], JSON_UNESCAPED_UNICODE),
            'chapter_structure' => $resultData['chapter_structure'] ?? 'linear',
            'cliffhanger_usage' => $resultData['cliffhanger_usage'] ?? 0,
            'interior_monologue' => $resultData['interior_monologue'] ?? 0,
            'description_density' => $resultData['description_density'] ?? 'moderate',
            'confidence' => $resultData['confidence'] ?? 0,
        ],
        'sentiment' => [
            'overall_tone' => $resultData['overall_tone'] ?? 'neutral',
            'emotional_range' => json_encode($resultData['emotional_range'] ?? [], JSON_UNESCAPED_UNICODE),
            'emotion_intensity' => $resultData['emotion_intensity'] ?? 'moderate',
            'depth_level' => $resultData['depth_level'] ?? 'entertaining',
            'thematic_complexity' => $resultData['thematic_complexity'] ?? 0,
            'themes' => json_encode($resultData['themes'] ?? [], JSON_UNESCAPED_UNICODE),
            'aesthetic_style' => $resultData['aesthetic_style'] ?? null,
            'beauty_description_focus' => json_encode($resultData['beauty_description_focus'] ?? [], JSON_UNESCAPED_UNICODE),
            'violence_level' => $resultData['violence_level'] ?? 'moderate',
            'moral_framework' => $resultData['moral_framework'] ?? null,
            'values_tendency' => json_encode($resultData['values_tendency'] ?? [], JSON_UNESCAPED_UNICODE),
            'confidence' => $resultData['confidence'] ?? 0,
        ],
        'creative_identity' => [
            'signature_phrases' => json_encode($resultData['signature_phrases'] ?? [], JSON_UNESCAPED_UNICODE),
            'unique_techniques' => json_encode($resultData['unique_techniques'] ?? [], JSON_UNESCAPED_UNICODE),
            'trademark_elements' => json_encode($resultData['trademark_elements'] ?? [], JSON_UNESCAPED_UNICODE),
            'genre_preferences' => json_encode($resultData['genre_preferences'] ?? [], JSON_UNESCAPED_UNICODE),
            'character_archetype_favorites' => json_encode($resultData['character_archetype_favorites'] ?? [], JSON_UNESCAPED_UNICODE),
            'plot_preferences' => json_encode($resultData['plot_preferences'] ?? [], JSON_UNESCAPED_UNICODE),
            'style_tags' => json_encode($resultData['style_tags'] ?? [], JSON_UNESCAPED_UNICODE),
            'influence_sources' => json_encode($resultData['influence_sources'] ?? [], JSON_UNESCAPED_UNICODE),
            'writing_voice' => $resultData['writing_voice'] ?? null,
            'editing_style' => $resultData['editing_style'] ?? 'moderate',
            'planning_style' => $resultData['planning_style'] ?? 'hybrid',
            'confidence' => $resultData['confidence'] ?? 0,
        ],
    };

    $table = $dimensionTables[$dimension];
    $existing = DB::fetch("SELECT id FROM `{$table}` WHERE profile_id=?", [$profileId]);
    if ($existing) {
        DB::update($table, $rowData, 'id=?', [$existing['id']]);
        return;
    }

    $rowData['profile_id'] = $profileId;
...
function handleGet(string $action, array $pathParts, ?int $userId): void
{
    ...
    if ($action === 'profile' && isset($pathParts[1])) {
        ...
        $profile = requireOwnedAuthorProfile($profileId, $userId);
    ...
    if ($action === 'works' && isset($pathParts[2])) {
        $profileId = intval($pathParts[2]);
        requireOwnedAuthorProfile($profileId, $userId);
        $works = DB::fetchAll(
...
function handlePut(string $action, array $pathParts, ?int $userId): void
{
    ...
    if ($action === 'dimension' && isset($pathParts[1]) && isset($pathParts[2])) {
        ...
        if (!is_array($resultData)) {
            authorJsonResponse(['error' => 'invalid_result_data'], 400);
        }

        saveAuthorDimensionResult($profileId, $dimension, $resultData);
        updateProfilePromptsFromResults($profileId, [$dimension => $resultData]);

        authorJsonResponse(['success' => true, 'dimension' => $dimension, 'result' => $resultData]);
    }
...
function handleDelete(string $action, array $pathParts, ?int $userId): void
{
    if ($action === 'profile' && isset($pathParts[1])) {
        ...
        // 审计修复 P2-1（2026-07-01）：IDOR 越权防护——校验画像归属权。
        // 任意登录用户枚举 id 即可删除他人画像的漏洞已封堵。
        $profileData = $profile->toArray();
        $profileUserId = $profileData['user_id'] ?? null;
        if ($userId !== null && $profileUserId !== null && (int)$profileUserId !== (int)$userId) {
            authorJsonResponse(['error' => '无权删除此画像'], 403);
        }

        $works = DB::fetchAll('SELECT file_path FROM author_uploaded_works WHERE profile_id=?', [$profileId]);
        foreach ($works as $work) {
            deleteManagedAbsoluteFile($work['file_path'] ?? null, BASE_PATH . '/storage/author_works');
        }
    ...
    if ($action === 'work' && isset($pathParts[1])) {
        ...
        // 审计修复 P2-1（2026-07-01）：IDOR 越权防护——通过 JOIN 校验作品归属权。
        $work = DB::fetch(
            'SELECT aw.*, ap.user_id AS profile_user_id
             FROM author_uploaded_works aw
             JOIN author_profiles ap ON aw.profile_id = ap.id
             WHERE aw.id=?',
            [$workId]
        );
        ...
        $workUserId = $work['profile_user_id'] ?? null;
        if ($userId !== null && $workUserId !== null && $workUserId !== $userId) {
            authorJsonResponse(['error' => '无权删除此作品'], 403);
        }
        ...
        // 清理引用该作品的画像的悬空 source_work_id，否则后续 analyze/analyze-dimension/
        // detailed-report 都会因作品已不存在而永久失败，且没有恢复路径。
        DB::update('author_profiles', ['source_work_id' => null, 'analysis_status' => 'pending'], 'source_work_id=?', [$workId]);
