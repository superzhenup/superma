...
require_once __DIR__ . '/../includes/ChapterMemoryFinalizer.php';

/**
 * 角色卡片为空时，从小说【已定义的角色】惰性补种卡片。
 *
 * 背景：角色卡片面板只读 character_cards 表，而该表仅由"写章节时 AI 摘要报告的
 * 角色变化"创建。结果：已定义主角(novels.protagonist_name)、配角(novel_characters)
 * 的小说，只要还没写出含角色变化的章节，面板就一直空白。
 *
 * 本函数把已定义的角色补种为卡片，让面板立即反映真实阵容。幂等：
 *  - 仅在当前【完全没有卡片】时执行（一次性补种，不覆盖 AI 已生成的卡片）；
 *  - upsert 按角色名去重，后续 AI 抽取会落在补种卡片上（增强而非重复）。
 *
 * @return int 补种的卡片数
 */
function seedCharacterCardsFromDefinitions(MemoryEngine $engine, int $novelId): int
{
    $repo = $engine->cards();
    if (!empty($repo->listAll(false))) return 0;  // 已有卡片 → 不补种

    $clip = fn($s, $n = 40) => mb_substr(trim((string)$s), 0, $n);
    $roleLabel = ['protagonist' => '主角', 'major' => '主要角色', 'minor' => '配角', 'background' => '背景角色'];
    $seeded = 0;

    // 1) 主角（novels.protagonist_name 必有定义）
    try {
        $novel = DB::fetch('SELECT * FROM novels WHERE id=?', [$novelId]);
        $pName = trim($novel['protagonist_name'] ?? '');
        if ($pName !== '') {
            $attrs = [];
            $pInfo = $clip($novel['protagonist_info'] ?? '', 60);  // 列可能不存在 → '' 跳过
            if ($pInfo !== '') $attrs['设定'] = $pInfo;
            $repo->upsert($pName, ['title' => '主角', 'attributes' => $attrs], 1);
            $seeded++;
        }
    } catch (\Throwable $e) { error_log('seed protagonist card failed: ' . $e->getMessage()); }

    // 2) 已定义的配角（novel_characters）
    try {
        $rows = DB::fetchAll(
            'SELECT name, role_type, gender, personality, abilities, appearance, first_chapter, first_appear
             FROM novel_characters WHERE novel_id=? ORDER BY id ASC LIMIT 60',
            [$novelId]
        );
        foreach ($rows as $r) {
            $name = trim($r['name'] ?? '');
            if ($name === '') continue;
            $attrs = [];
            if ($clip($r['gender'] ?? '', 10) !== '')   $attrs['性别'] = $clip($r['gender'], 10);
            if ($clip($r['personality'] ?? '') !== '')  $attrs['性格'] = $clip($r['personality']);
            if ($clip($r['abilities'] ?? '') !== '')    $attrs['能力'] = $clip($r['abilities']);
            if ($clip($r['appearance'] ?? '') !== '')   $attrs['外貌'] = $clip($r['appearance']);
            $title = $roleLabel[$r['role_type'] ?? 'minor'] ?? '配角';
            $chNum = (int)($r['first_chapter'] ?? $r['first_appear'] ?? 1);
            if ($chNum <= 0) $chNum = 1;
            $repo->upsert($name, ['title' => $title, 'attributes' => $attrs], $chNum);
            $seeded++;
        }
    } catch (\Throwable $e) { error_log('seed novel_characters cards failed: ' . $e->getMessage()); }
...
try {
    switch ($action) {
        ...
        case 'get_cards':
            // 修复：面板为空时，从小说已定义的角色（主角 + novel_characters）惰性补种卡片，
            // 避免"尚未写出含角色变化的章节 → 角色卡片永远空白"。仅在无任何卡片时执行一次。
            try { seedCharacterCardsFromDefinitions($engine, $novelId); }
            catch (\Throwable $e) { error_log('get_cards seed skip: ' . $e->getMessage()); }
        ...
        case 'search_atoms':
            ...
            // 审计修复 P2-5（2026-07-01）：补 limit 上限和 type 白名单校验
            // 对齐 get_atoms 的 min(..., 200) 规范，防止 limit=1000000 拖垮 DB
            $limit = max(1, min(200, (int)($input['limit'] ?? 10)));
            $type = !empty($input['atom_type'])
                ? (in_array($input['atom_type'], AtomRepo::VALID_TYPES, true) ? (string)$input['atom_type'] : null)
                : null;
        ...
        case 'extract_chapter':
            ...
            // 查章节内容
            $chapter = DB::fetch(
                'SELECT id, chapter_number, content, chapter_summary, status
            ...
            if (($chapter['status'] ?? '') !== 'completed') {
                echo json_encode(['ok' => false, 'msg' => '章节尚未完成，禁止提取部分正文记忆'], JSON_UNESCAPED_UNICODE);
                break;
            }
            // 尝试从 chapter_summary 重建记忆（老版本可能是JSON，新版本是纯文本）
            $summary = null;
            if (!empty($chapter['chapter_summary'])) {
                $decoded = json_decode($chapter['chapter_summary'], true);
                if (is_array($decoded)) $summary = $decoded;
            }
            $chapterContent = (string)($chapter['content'] ?? '');
            if (trim($chapterContent) === '') {
                echo json_encode(['ok' => false, 'msg' => '章节暂无内容，无法提取'], JSON_UNESCAPED_UNICODE);
                break;
            }

            // 新版纯文本摘要直接组装最小结构，避免再次调用 AI；所有写入仍走
            // 同一个哈希/事务/幂等边界，不能直接 append atom 或二次 ingest。
            if ($summary === null) {
                $summaryText = trim((string)($chapter['chapter_summary'] ?: $chapterContent));
                $snippet = mb_substr($summaryText, 0, 300);
                $summary = [
            ...
            $result = ChapterMemoryFinalizer::finalize(
                $novelId,
                $chapterId,
                $chapterContent,
                $engine,
            ...
            $message = !empty($result['duplicate'])
                ? '该正文版本已提取，无需重复写入'
                : (!empty($result['historical_deferred'])
                    ? '该章之后已有完成章节；为避免污染较新状态，请按章节顺序重放记忆'
                    : (!empty($result['stale']) ? '正文已变化，本次提取已取消' : '记忆提取完成'));
            echo json_encode([
                'ok' => !empty($result['ok']),
                'msg' => $message,
                'data' => $result['ingest'] ?? [],
