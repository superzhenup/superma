...
try {
    ...
    // 审计修复 P2-3（2026-07-01）：不向客户端透传原始异常消息，防止泄露 DB 结构/文件路径等内部细节
    $rid = error_trace_id();
    error_log(sprintf('[%s] iterative_monitor: %s in %s:%d', $rid, $e->getMessage(), $e->getFile(), $e->getLine()));
...
/**
 * 审计修复（2026-07-19 H-13）：统一解包 iterative_history。
 * 存储端（IterativeRefinementController::saveIterationHistory）写入的是
 * {"iterations":[...], "improvement_trend":..., ...} 包装对象，
 * 而本文件各消费端历史上直接按平铺迭代数组遍历，导致监控统计全部失效。
 * 兼容旧格式（平铺数组）与新格式（包装对象）。
 */
function unwrapIterationHistory(?string $raw): array
{
    if ($raw === null || $raw === '') return [];
    $history = json_decode($raw, true);
    if (!is_array($history)) return [];
    if (isset($history['iterations']) && is_array($history['iterations'])) {
        return $history['iterations'];
    }
    // 平铺数组（旧格式）判定：存在数值键 0
    return array_key_exists(0, $history) ? $history : [];
}
...
function analyzeCommonIssues(int $novelId): array
{
    try {
        ...
        foreach ($chapters as $chapter) {
            $history = unwrapIterationHistory($chapter['iterative_history'] ?? null);
            if (!$history) continue;
...
function analyzeImprovementPatterns(int $novelId): array
{
    try {
        $chapters = DB::fetchAll(
            'SELECT iterative_history, total_improvement FROM chapters
             WHERE novel_id = ? AND rewritten = 1 AND iterative_history IS NOT NULL',
            [$novelId]
        );
