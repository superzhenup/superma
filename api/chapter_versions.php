...
// 获取章节信息
$chapter = DB::fetch('SELECT id, chapter_number, title, content, words, version FROM chapters WHERE id=?', [$chapterId]);
if (!$chapter) {
...
if ($action === 'list') {
    ...
    jsonResponse(true, [
        'chapter' => [
            ...
            'current_content' => mb_substr($chapter['content'] ?? '', 0, 500) . (mb_strlen($chapter['content'] ?? '') > 500 ? '...' : '')
        ],
