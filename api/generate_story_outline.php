...
// 否则前端断线后脚本继续空烧 AI token + 前端重试放大消耗。
...
// 检查是否已存在故事大纲——重新生成时旧记录延迟到新大纲通过校验后才删除
// （审计修复：原逻辑在此处提前删除旧记录，若 AI 后续返回畸形/近空 JSON，
// empty() 校验无法拦截，会导致旧大纲永久丢失且被空白记录替换）
$existing = DB::fetch('SELECT id FROM story_outlines WHERE novel_id=?', [$novelId]);
if ($existing) {
    sse('progress', ['msg' => '检测到已有故事大纲，正在重新生成...']);
}
...
// 审计修复：is_array 非空不足以判断有效性（AI 可能返回 {"note":"..."} 之类无实质内容的
// JSON），必须核实关键字段存在，否则会把旧大纲替换成空白记录且无法恢复。
if (empty($storyOutline) || (empty($storyOutline['story_arc']) && empty($storyOutline['act_division']))) {
    sse('error', [
...
// ---- 保存到数据库 ----
// M-6 修复：delete+insert 包裹事务，防止中途崩溃导致大纲丢失；
// 延迟到此处（新大纲已通过上方字段校验）才删除旧记录，避免畸形 AI 响应清空旧大纲。
try {
    if ($existing) {
        DB::beginTransaction();
