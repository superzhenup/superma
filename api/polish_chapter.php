...
if (!empty($oldContent) && $oldWords > 100) {
    ...
    $sse->msg(['version_saved' => true, 'version' => $maxVer + 1, 'words' => $oldWords]);
...
try {
    withModelFallback($novel['model_id'] ?? null, function($ai) use ($polishSystem, $polishUser, &$fullContent, &$usedModel, $sse) {
