...
// 生产环境隐藏诊断端点：未开启时直接 404，避免接口被枚举。
if (!CFG_DIAGNOSTIC_ENABLED) {
    http_response_code(404);
    exit;
}

// 审计修复 P2-15（2026-07-12）：诊断端点限制仅管理员可访问
if (!isAdmin()) {
    http_response_code(403);
    echo json_encode(['ok' => false, 'error' => '仅管理员可访问修复工具'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
if (strpos($enumStr, 'cool_point') === false) {
    ...
    if (!$dryRun) {
        try {
            ...
            error_log('fix_semantic ENUM alter failed: ' . $e->getMessage());
            $result['actions'][count($result['actions'])-1]['ok'] = false;
            // 审计修复 P2-15（2026-07-12）：不向客户端泄露数据库异常细节
            $result['actions'][count($result['actions'])-1]['error'] = '修复失败，请查看错误日志';
        }
    }
} else {
