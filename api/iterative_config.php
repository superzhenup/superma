...
// 审计修复（2026-07-19 M3-NEW3）：写操作与历史查询必须绑定有效小说并已通过归属校验。
// novel_id<=0 时 checkNovelOwnership 不会执行，任意登录用户可读写 novel_id=0 的共享配置行（跨用户写）。
// defaults 动作仅返回 schema 默认值不涉及小说数据，允许 novel_id=0。
$writeBoundActions = ['update', 'history'];
if (in_array($action, $writeBoundActions, true) && $novelId <= 0) {
...
function updateIterativeConfig(int $novelId, array $input)
{
    try {
        ...
        try { DB::rollBack(); } catch (\Throwable $e2) { error_log('iterative_config rollback failed: ' . $e2->getMessage()); }
...
/**
 * 按 schema 规则强制类型转换并钳制范围。
 * 审计修复 P2-5（2026-07-01）：避免 threshold=99999、enabled="string"、
 * max_iterations=-5 等越界/类型错误输入写入 DB 破坏类型契约。
 */
function coerceSettingValue($value, array $rule)
{
    ...
    if ($type === 'number') {
        $v = (float)$value;
        if (isset($rule['min'])) $v = max((float)$rule['min'], $v);
        if (isset($rule['max'])) $v = min((float)$rule['max'], $v);
        // 无 step 字段表示整数字段，保留 int 类型
        if (!isset($rule['step'])) {
            return (int)$v;
        }
        return $v;
    ...
    if ($type === 'int') {
        $v = (int)$value;
        if (isset($rule['min'])) $v = max((int)$rule['min'], $v);
        if (isset($rule['max'])) $v = min((int)$rule['max'], $v);
        return $v;
    }
    // string 兜底：截断到 1000 字符，防止超长输入
...
{
    // 审计修复 P2-5（2026-07-01）：按 schema 校验输入，强制类型转换、范围钳制，丢弃未知键
    $schema = getDefaultIterativeConfig()[$key] ?? [];
    // schema 未声明但已知合法的扩展键及其校验规则
    $extraRules = [
        'rewrite' => ['rewrite_model_id' => ['type' => 'int', 'min' => 0, 'max' => 100000]],
        'iterative_refinement' => [],
    ];
    $rules = array_merge($schema, $extraRules[$key] ?? []);
    ...
    foreach ($values as $settingKey => $value) {
        ...
        // 未知键或缺少 type 声明：丢弃，不再透传到 DB
        if (!isset($rules[$settingKey]) || !is_array($rules[$settingKey]) || !isset($rules[$settingKey]['type'])) {
            continue;
        }
        $settingValue[$settingKey] = coerceSettingValue($value, $rules[$settingKey]);
    }
    ...
    // 修复：iterative_settings 写入成功后失效 getSetting 请求级缓存（2026-07-22），
    // 保证同一请求内后续读取（如 RewriteAgent 重新实例化）拿到新值。
    clearGetSettingCache();

    // 审计修复（2026-07-19 H-中10）：重写开关由系统级（污染所有小说）改为小说级，
    // 写入 iterative_settings 表的 rewrite.* 键，与 RewriteAgent 读取路径一致。
    // 不再写入全局 system_settings 的 ws_rewrite_enabled 等 5 个键。
    // （注：此函数仍在 system_settings 上下文被调用，但改写后的 rewrite 开关仅对当前
    //  $novelId 生效——在 saveSetting 函数顶部已有 $novelId > 0 的校验。）
}
