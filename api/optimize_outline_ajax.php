...
// 审计修复（2026-07-19 H-中14）：负数 batch_index 会使 array_slice 取末批，
// 进度被标记"已完成"但实际未处理。钳制到 ≥0。
...
// 审计修复 IDOR（2026-06-18）：归属校验，防止跨用户优化他人小说大纲
...
if (empty($chapters)) {
    echo json_encode(['success' => false, 'message' => '暂无可优化的未完成章节大纲']);
    exit;
}
...
try {
    ...
    // 审计修复 P2-3（2026-07-01）：不向客户端透传模型配置异常细节
    $rid = error_trace_id();
    error_log(sprintf('[%s] optimize_outline_ajax: %s in %s:%d', $rid, $e->getMessage(), $e->getFile(), $e->getLine()));
    echo json_encode(['success' => false, 'message' => '模型配置异常，请检查模型设置', 'code' => 'business_error', 'request_id' => $rid]);
    exit;
}

$totalChapters = count($chapters);
// 审计 P3-E（2026-06-11）：批次大小统一读集中配置（原硬编码 10 与 CFG_OPTIMIZE_BATCH 脱节，
// 改配置会导致前后端批次位置换算错位）。响应均携带 batch_size 供前端换算。
$batchSize = max(1, min(50, (int)(defined('CFG_OPTIMIZE_BATCH') ? CFG_OPTIMIZE_BATCH : 10)));

// 计算批次范围
$batchStart = $batchIndex * $batchSize;
if ($batchStart >= $totalChapters) {
    // 所有批次已完成
    $lastOptimizedChapter = (int)(end($chapters)['chapter_number'] ?? 0);
    DB::update('novels', ['optimized_chapter' => $lastOptimizedChapter], 'id=?', [$novelId]);
    echo json_encode([
        'success' => true,
        'message' => '所有章节优化完成',
        'progress' => [
...
// 解析 AI 响应
// 审计修复：改用与 optimize_outline.php 一致的 extractOutlineObjects()，
// 而非本地手写的 json_decode——后者缺少字段别名规范化(normalizeOutlineObject)
// 与截断 JSON 逐对象兜底提取，会导致 AI 使用别名字段（如"章节号"）时该章节被静默跳过。
$batchResult = extractOutlineObjects($rawResponse);
...
foreach ($batchResult as $item) {
    ...
    try {
        $mutation = ChapterMutationService::mutateChapter(
            (int)$existing['id'],
            $novelId,
            $updateData,
            [
                ...
                'allowed_statuses' => ['pending', 'outlined', 'skipped', 'failed'],
                'reason' => 'outline_logic_optimization',
            ]
        );
...
echo json_encode([
    ...
    'progress' => [
        'current' => $batchEnd,
        'total' => $totalChapters,
        'percent' => round(($batchEnd / $totalChapters) * 100, 1)
    ],
    'batch_result' => [
        'from' => $batchFrom,
        'to' => $batchTo,
        'updated' => $updatedCount,
        'changed' => array_values(array_filter($batchResult, fn($item) => !empty($item['changed']))),
    ...
    // 最后一批直接报告完成：前端循环靠 has_more=false 退出，
    // 不会再发“越界的下一批”请求，缺这个字段就永远走不到完成分支
    'completed' => ($batchEnd >= $totalChapters)
], JSON_UNESCAPED_UNICODE);
