...
if (empty($edgeMap)) {
    try {
        ...
        $chRows = DB::fetchAll(
            'SELECT chapter_number, content FROM chapters WHERE novel_id=? ORDER BY chapter_number',
            [$novelId]
        );
        ...
        if (count($chRows) === 0) {
            ...
            foreach ($chRows as $ch) {
                $content = $ch['content'] ?? '';
                foreach ($charMap as $cName => $cData) {
                    if (!empty($cData['_merged_into'])) continue;
            ...
            if ($charsInChapters < 2) {
                ...
                $cooccurrenceMin = max(3, intval(count($chRows) * 0.03));
