...
$defaultModelSupports1M = false;
foreach ($models as &$modelRow) {
    $modelCaps = json_decode((string)($modelRow['capabilities'] ?? '[]'), true);
    $modelRow['_supports_context_1m'] = is_array($modelCaps)
        && in_array('context_1m', $modelCaps, true);
    if (!empty($modelRow['is_default']) && $modelRow['_supports_context_1m']) {
        $defaultModelSupports1M = true;
    }
}
unset($modelRow);
                ...
                <div class="a-card">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="bookModel">
                            <option value="" data-context-1m="<?= $defaultModelSupports1M ? '1' : '0' ?>">默认模型</option>
                            <?php foreach ($models as $m): ?>
                            <option value="<?= $m['id'] ?>"
                                    data-context-1m="<?= !empty($m['_supports_context_1m']) ? '1' : '0' ?>"
...
async function handleFile(file) {
    ...
    try {
        const res = await fetch(apiRouteUrl('analyze_book', 'action=upload_txt'), {method:'POST', body:fd});
...
function updateCharCounter() {
    ...
    const is1MModel = selectedOption && selectedOption.dataset.context1m === '1';
...
async function runStep(step) {
    ...
    try {
        const response = await fetch(apiRouteUrl('analyze_book', 'action=analyze_step'), {
            method: 'POST',
...
async function doRewrite() {
    ...
    try {
        const res = await apiPost(apiRouteUrl('analyze_book', 'action=rewrite'), {
...
async function saveAnalysis() {
    ...
    try {
        const res = await apiPost(apiRouteUrl('analyze_book', 'action=save'), {
            title, content,
            author: document.getElementById('bookAuthor').value.trim(),
            genre: document.getElementById('bookGenre').value,
...
async function loadHistory() {
    try {
        const res = await apiPost(apiRouteUrl('analyze_book', 'action=list'), {});
...
async function viewHistory(id) {
    try {
        const res = await apiPost(apiRouteUrl('analyze_book', 'action=get&id=' + encodeURIComponent(id)), {});
...
async function deleteHistory(id) {
    ...
    try { await apiPost(apiRouteUrl('analyze_book', 'action=delete'), {id}); showToast('已删除','success'); loadHistory(); } catch(e) {}
}
