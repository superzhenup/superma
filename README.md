# ✦ Super Ma Agents v1.8.0 — AI 智能小说创作辅助系统（平台）
## UI预览
...
从数据库结构版本 53 开始，普通 Web 请求和写作 worker 不再自动执行 DDL。升级代码后必须由管理员在项目根目录显式运行：
...
章节正文落盘后会先写入 `postprocess_jobs`，当前进程会立即尝试执行；失败任务由 CLI worker 按指数退避重试。建议每分钟执行一次：

```bash
php bin/postprocess_worker.php --max=10
```

完整备份包含 MySQL 数据库、封面和作者上传作品。备份目录被强制要求位于 Web 根目录之外：

```bash
php bin/backup.php --output=/srv/backups/super-ma
php bin/restore.php --from=/srv/backups/super-ma/super-ma-backup-YYYYmmdd-HHMMSS-xxxxxx --confirm=RESTORE
```

恢复前会验证清单中每个文件的 SHA-256，并默认先生成一次恢复前安全备份。默认合并恢复存储文件；只有确实需要让存储目录与备份完全一致时，才追加 `--replace-files`。`mysqldump` 和 `mysql` 不在 PATH 时，可分别使用 `--mysqldump=/绝对路径`、`--mysql=/绝对路径`。

Apache 已通过 `.htaccess` 禁止浏览器访问 `bin/`。Nginx 部署必须补充同等规则：
...
- **修复所有已知问题**：修复所有已知问题，提升写作稳定性

**性能优化**

- **提升软件性能**：提升软件性能，优化写作质量
...
**支持大模型1M 专项优化**
- 在模型设置中显式勾选“支持 1M 上下文”；不再依赖模型名称标签，避免误识别
- 大纲生成：批量数从 5 章提升至 30 章，PHP 超时从 600s 提升至 1200s
- 章节写作：自动启用完整上下文模式，在安全预算内优先注入历史大纲、正文、摘要与关键状态
...
GPLv3 with additional commercial-use restriction (see LICENSE Section 7)
（允许个人学习研究，支持个人创作者创作文本内容盈利，企业及组织商用需得到授权）
