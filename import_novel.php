...
const BATCH_SIZE = 50;
const API = 'api/import_novel_excel.php';
...
async function startImport() {
  ...
  localStorage.setItem('novel_import_session', JSON.stringify({ sessionKey, novelId: importState.novelId }));
  ...
  localStorage.removeItem('novel_import_session');
...
(function checkResume() {
  const stored = localStorage.getItem('novel_import_session');
  if (!stored) return;
  try {
    ...
    if (s.sessionKey && s.novelId) {
      ...
      document.getElementById('btnCancelResume').onclick = () => {
        localStorage.removeItem('novel_import_session'); bar.remove();
      };
    }
  } catch(e) {}
})();
