...
// ============================================================
if ($action === 'check') {
    // 审计 2026-06-11 P2：该端点接受 POST 但既往无 CSRF 校验，
    // 攻击者可借登录用户会话发起跨站请求消耗 AI token quota。
    // 该文件独立于 api/index.php 路由分发器，需自行调用 csrf_verify_api()。
    // 使用 csrf_verify_api() 而非 csrf_verify()，因为此端点是 JSON AJAX 接口，
    // 前端通过 fetch().json() 解析响应，需要结构化 JSON 错误而非 die() 纯文本。
    csrf_verify_api();
