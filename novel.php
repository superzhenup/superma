<?php
// 错误处理：捕获所有致命错误
// 审计 2026-06-11 P1：原实现把 $error['message']/['file']/['line'] 直接拼成 HTML 输出，
// 泄露内部路径与代码结构。统一改为与 includes/error_handler.php 一致的收口策略：
// 完整细节只写 error_log，客户端仅看到友好文案 + 可追踪请求号。
register_shutdown_function(function () {
    ...
    if ($error && in_array($error['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR], true)) {
        ...
        // 追踪号：与 includes/helpers.php 的 error_trace_id() 同语义；
        // 此处独立生成兜底，避免 helpers.php 尚未加载时报未定义函数。
        $rid = '';
        try {
        ...
        echo "<!DOCTYPE html>\n";
        echo "<html><head><meta charset='UTF-8'><title>服务器错误</title></head><body>\n";
        echo "<h1>服务器内部错误</h1>\n";
        echo "<p>页面加载失败，请稍后重试。</p>\n";
        echo "<p style=\"color:#999;font-size:.9em\">追踪号：" . htmlspecialchars($rid, ENT_QUOTES, 'UTF-8') . "</p>\n";
        echo "</body></html>\n";
        exit;
    }
});

// 禁止向页面输出错误，由已注册的错误处理函数记录
...
// 所有按 novel_id 执行的恢复、暂停、画像读取都可能产生副作用，必须先
// 完成页面级归属校验，不能仅在页面渲染前补一道迟到的检查。
$novel = getNovel($id);
if (!$novel) {
...
// 修复（2026-06-18）：页面刷新时校正 writing 状态
// 场景：用户点击"自动写作" → novels.status='writing' → 用户刷新页面（未点暂停）
// 前端 JS 状态丢失，无法恢复对后台 worker 的控制（SSE/轮询连接已断开）
// 修复：刷新时若 status=writing，直接重置为 paused，按钮显示"自动写作"
// 若后台 worker 仍在运行，用户点击"自动写作"后 write_start 会检测到已有任务
// 并返回 resumed=true，前端恢复轮询（见 write_start.php 修复）
if ($novel && ($novel['status'] ?? '') === 'writing') {
    DB::update('novels', ['status' => 'paused'], 'id=?', [$id]);
    $novel['status'] = 'paused';
}
...
// [v42] 性能优化（审计 2026-06-10 P1-A）：本页只需要章节号/状态做计数与下拉，
// 原先无分页地全量拉取所有章节（含正文 LONGTEXT），千章书一次数十 MB。
// 改为一条轻查询 + 分页查询，整本书正文不再进入页面渲染路径。
$chapterIndex = DB::fetchAll(
    'SELECT chapter_number, status FROM chapters WHERE novel_id=? ORDER BY chapter_number ASC',
    [$id]
);
$models = DB::fetchAll('SELECT * FROM ai_models ORDER BY is_default DESC, id ASC');

// 章节分页：50章/页
$totalChapterCount = count($chapterIndex);
$perPage           = 50;
...
$outlined  = count(array_filter($chapterIndex, fn($c) => in_array($c['status'], ['outlined','writing','completed'])));
$completed = count(array_filter($chapterIndex, fn($c) => $c['status'] === 'completed'));
  ...
  <div class="d-flex align-items-start gap-4 flex-wrap">
    ...
    <div class="flex-grow-1 min-w-0">
      ...
      <div class="d-flex gap-2 flex-wrap">
        ...
        <a href="api/index.php?route=export_novel&amp;id=<?= $id ?>" class="btn btn-sm btn-outline-success" <?= $completed === 0 ? 'disabled title="暂无已完成的章节"' : '' ?>>
          ...
          <div class="d-flex justify-content-between align-items-center mb-3">
            <div class="d-flex gap-2">
              ...
              <select class="form-select form-select-sm" id="atomChapterFilter" style="width: auto;">
                ...
                <?php foreach ($chapterIndex as $ch): ?>
...
const MemoryUI = {
  ...
  async loadCards() {
    ...
    try {
      ...
      if (!res.ok) {
        container.innerHTML = `<div class="col-12 text-center text-danger py-4"><p>加载失败：${escHtml(res.msg || '未知错误')}</p></div>`;
        return;
      }
      ...
      container.innerHTML = `<div class="col-12 text-center text-danger py-4"><p>加载失败：${escHtml(e.message)}</p></div>`;
    }
  },
  ...
  async loadAtoms(offset = 0) {
    ...
    try {
      ...
      if (!res.ok) {
        container.innerHTML = `<div class="text-center text-danger py-4"><i class="bi bi-exclamation-circle fs-1 d-block mb-2"></i><p>加载失败：${escHtml(res.msg || '未知错误')}</p></div>`;
        return;
      }
      ...
      container.innerHTML = `<div class="text-center text-danger py-4"><i class="bi bi-exclamation-circle fs-1 d-block mb-2"></i><p>加载失败：${escHtml(e.message)}</p></div>`;
    }
  },
  ...
  async loadClusters() {
    ...
    try {
      ...
      if (!res.ok) {
        container.innerHTML = `<div class="text-center text-danger py-4"><p>加载失败：${escHtml(res.msg || '未知错误')}</p></div>`;
        return;
      }
      ...
      container.innerHTML = `<div class="text-center text-danger py-4"><p>加载失败：${escHtml(e.message)}</p></div>`;
    }
  },
  ...
  loadAutoExtractSetting() {
    let saved = null;
    try { saved = localStorage.getItem(`novel_${NOVEL_ID}_auto_extract`); } catch (_) {}
  ...
  saveAutoExtractSetting(enabled) {
    try { localStorage.setItem(`novel_${NOVEL_ID}_auto_extract`, enabled); } catch (_) {}
  },
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btn-save-human-critic')?.addEventListener('click', async function() {
    ...
    try {
      var res = await fetch(apiRouteUrl('human_critic'), {
        method: 'POST',
      ...
      if (data.ok) {
        ...
        status.innerHTML = '<span class="text-danger">' + escHtml(data.msg || '保存失败') + '</span>';
      }
    } catch (e) {
  ...
  async function loadHumanCritic(chapterId) {
    var dims = ['thrill', 'immersion', 'pacing', 'freshness', 'read_next'];
    try {
      var res = await fetch(apiRouteUrl('human_critic', 'action=get&novel_id=' + encodeURIComponent(NOVEL_ID) + '&chapter_id=' + encodeURIComponent(chapterId)), {
  ...
  // 取消写作（重置章节状态）：唯一实现在 app.js 的 window.resetSingleChapter
  // （审计 P3-D，2026-06-11：此处原有的重复定义与 app.js 版本行为漂移，已收敛删除）
});
...
const DaemonWrite = {
  ...
  async toggle() {
    ...
    if (enabled) {
      ...
      if (token) {
        try {
          const res = await fetch('api/daemon_write.php?action=status', {
  ...
  async start() {
    ...
    try {
      const res = await fetch('api/daemon_write.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded', 'X-Daemon-Token': token, 'X-CSRF-Token': getCsrf()},
        body: new URLSearchParams({ novel_id: String(NOVEL_ID), action: 'enable' })
      });
      const d   = await res.json();
  ...
  async stop() {
    ...
    try {
      const res = await fetch('api/daemon_write.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded', 'X-Daemon-Token': token, 'X-CSRF-Token': getCsrf()},
        body: new URLSearchParams({ novel_id: String(NOVEL_ID), action: 'disable' })
      });
      const d   = await res.json();
  ...
  _updateCurlCmd(token) {
    const daemonUrl = new URL('api/daemon_write.php', window.location.href).toString();
    const cmd    = `curl -s -H "X-Daemon-Token: ${token}" "${daemonUrl}"`;
  ...
  async _poll(token) {
    try {
      const res = await fetch(`api/daemon_write.php?novel_id=${NOVEL_ID}&action=status`, {
        headers: {'X-Daemon-Token': token}
      });
      const d   = await res.json();
...
function showCoverModal(novelId) {
    ...
    var currentCover = <?= json_encode($novel['cover_image'] ?? '', JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
    var coverColor = <?= json_encode($novel['cover_color'] ?? '#6366f1', JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
    var title = <?= json_encode($novel['title'], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
    ...
    <div class="modal-dialog">
      <div class="modal-content bg-dark text-light">
        <div class="modal-header border-secondary">
          <h5 class="modal-title"><i class="bi bi-image me-2"></i>管理封面 - ${_escAttr(title)}</h5>
          <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
...
function uploadCoverFromModal(novelId) {
    ...
    fetch(apiRouteUrl('cover_actions'), { method: 'POST', body: fd, headers: {'X-CSRF-Token': getCsrf()} })
    .then(r => r.json())
    .then(data => {
        if (data.ok) {
            status.className = 'small text-success';
            status.innerHTML = '<i class="bi bi-check-circle me-1"></i>' + escHtml(data.msg);
            ...
            status.innerHTML = '<i class="bi bi-x-circle me-1"></i>' + escHtml(data.msg || '上传失败');
        }
    })
    .catch(err => {
        status.className = 'small text-danger';
        status.innerHTML = '<i class="bi bi-x-circle me-1"></i>' + escHtml(err.message);
    });
}
...
function generateCoverFromModal(novelId) {
    ...
    fetch(apiRouteUrl('cover_actions'), {
        method: 'POST',
    ...
    .then(data => {
        if (data.ok) {
            ...
            status.className = 'small text-danger';
            status.innerHTML = '<i class="bi bi-x-circle me-1"></i>' + escHtml(data.msg || '生成失败');
        }
    })
    .catch(err => {
        status.className = 'small text-danger';
...
function deleteCoverFromModal(novelId) {
    ...
    .then(data => {
        if (data.ok) {
            ...
            var coverColor = <?= json_encode($novel['cover_color'] ?? '#6366f1', JSON_HEX_TAG | JSON_HEX_AMP) ?>;
            var title = <?= json_encode($novel['title'], JSON_HEX_TAG | JSON_HEX_AMP) ?>;
            ...
            var mainCover = document.querySelector('.novel-cover-sm');
            if (mainCover) {
                mainCover.style.background = 'linear-gradient(135deg,' + _escAttr(coverColor) + ',' + _escAttr(coverColor) + '99)';
                mainCover.textContent = title.substring(0, 4);
            }
        } else {
            status.className = 'small text-danger';
            status.innerHTML = '<i class="bi bi-x-circle me-1"></i>' + escHtml(data.msg || '删除失败');
        }
    })
    .catch(err => {
        status.className = 'small text-danger';
...
const AgentPanel = {
  ...
  async loadAll() {
    this.showLoading();
    try {
      const res = await fetch(apiRouteUrl('agent_status', 'novel_id=' + encodeURIComponent(NOVEL_ID)));
...
<?php
// 审计修复 M-06（2026-06-12）：调试内存信息改用 error_log
if (defined('APP_DEBUG') && APP_DEBUG) {
    $memUsed = round(memory_get_usage(true) / 1024 / 1024, 2);
    $memPeak = round(memory_get_peak_usage(true) / 1024 / 1024, 2);
    error_log("[novel.php] Final Memory Usage: {$memUsed}MB / Peak: {$memPeak}MB");
}
